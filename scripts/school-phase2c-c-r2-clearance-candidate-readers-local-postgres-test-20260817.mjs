...
const root = mkdtempSync(join(tmpdir(), "school-phase2c-c-r2-"));
...
try {
  ...
  const migration = file("sql/current/school_phase2c_c_r2_clearance_candidate_readers_migration_20260817.sql", {
    PHASE2C_C_R2_SKIP_PRODUCTION_MD5: "1",
  });
  assert.match(migration, /COMMIT/);
  const contract = file("sql/tests/school_phase2c_c_r2_clearance_candidate_readers_local_20260817.sql");
  assert.match(contract, /passed_assertions=13/);
  const roles = file("sql/tests/school_phase2c_c_r2_clearance_candidate_readers_role_matrix_local_20260817.sql");
  assert.match(roles, /PHASE2C_C_R2_ROLE_MATRIX_PASS/);

  assert.equal(scalar("select count(*) from school_lesson_clearances"), "2");
  file("sql/current/school_phase2c_c_r2_clearance_candidate_readers_exact_rollback_20260817.sql", {
    PHASE2C_C_R2_SKIP_PRODUCTION_MD5: "1",
  });
  assert.equal(scalar("select to_regprocedure('school_list_lesson_clearance_pending_balances_v2(uuid,boolean)') is null"), "t");
  assert.equal(scalar("select count(*) from school_lesson_clearances"), "2");
  console.log("SCHOOL_PHASE2C_C_R2_CLEARANCE_CANDIDATE_READERS_LOCAL_POSTGRES_PASS");
} finally {
