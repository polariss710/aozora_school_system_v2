...
// js/api/settlement-api.js 的模块作用域，不再对外导出。任何模块——包括测试——
// 都无法自行登记一个对象。这正是 P0 金额边界要的来源约束：
// buildOnlineDraftLockInput 认可的快照只可能来自 DB 读取。
//
// 代价是原本零依赖的页面层单元测试现在要拖上 supabase 客户端。这是那条约束的
// 内在成本，不是可以绕开的配置问题。
