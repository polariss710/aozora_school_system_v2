-- =============================================================================
-- 回滚：撤除 5 个收入 writer 与学费 G 的 operator 准入守卫
--
...
-- ⚠️ 回滚【不依赖】operator 守卫：6 个 canonical 里没有一处引用它，
--    回滚做的正是把守卫调用拿掉。若守卫因故消失，6 个函数会在运行时报
--    「函数不存在」，收入与学费生成全线中断，而回滚是唯一的救援手段。
--    因此这里只告警、不中断 —— 在最需要退路的时候把退路锁死是更坏的结果。
...
BEGIN
  ...
  IF v_n <> 1 THEN
    RAISE WARNING 'OPWRI_RB_GUARD_MISSING: public.school_require_current_app_operator 有 % 个（应为 1）', v_n;
    RETURN;
  END IF;
  ...
  IF v_md5 <> '526c615da0c5bf179894ccbcc325008c' THEN
    RAISE WARNING 'OPWRI_RB_GUARD_DRIFT: 守卫定义为 %，期望 %', v_md5, '526c615da0c5bf179894ccbcc325008c';
    RETURN;
  END IF;
  IF v_acl <> '{postgres=X/postgres,authenticated=X/postgres}' THEN
    RAISE WARNING 'OPWRI_RB_GUARD_ACL: 守卫 ACL 为 %，期望 %', v_acl, '{postgres=X/postgres,authenticated=X/postgres}';
    RETURN;
  END IF;
  RAISE NOTICE 'OPWRI_RB: operator 守卫状态正常（回滚本身不依赖它）';
END
$guard$;

-- 当前必须【正是本次部署的结果】。若已是基线则明确报出，不做无谓写入。
DO $already$
DECLARE v_n int;
BEGIN
  SELECT count(*) INTO v_n FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
   WHERE n.nspname='public' AND p.proname IN ('school_create_income_record','school_update_income_record','school_reverse_income_record','school_cancel_pending_income_record','school_create_pending_cash_income_record','school_generate_student_tuition_bill_atomic')
     AND md5(pg_get_functiondef(p.oid)) IN ('9d141f81ed1aba0c4c4e440ff4cc3c14','a5189e72f4ce4e692afb746c107d2241','60005d5436e74165ae3d23749fed823b','078349ebbe8c3be3d93aa505eac3b1ef','191350ab8a1bbb93254b8c1142d4b64b','88b6d181d6b5eb83c7eceb37c056a031');
  IF v_n = 0 THEN
    RAISE EXCEPTION 'OPWRI_ALREADY_BASELINE: 6 个函数均非部署后状态，无需回滚';
  END IF;
END
$already$;
...
BEGIN
  ...
  LOOP
    ...
    IF v_n <> 1 THEN
      RAISE EXCEPTION 'OPWRI_RB_ARITY: public.%(%) 匹配到 % 个（应为 1）', t.proname, t.nargs, v_n;
    END IF;
    ...
    IF v_md5 <> t.md5 THEN
      RAISE EXCEPTION 'OPWRI_RB_MD5: %(%) 定义为 %，期望 %', t.proname, t.nargs, v_md5, t.md5;
    END IF;
    IF v_acl <> t.acl THEN
      RAISE EXCEPTION 'OPWRI_RB_ACL: %(%) ACL 为 %，期望 %', t.proname, t.nargs, v_acl, t.acl;
    END IF;
    IF v_cmt <> t.cmt THEN
      RAISE EXCEPTION 'OPWRI_RB_COMMENT: %(%) 注释为 [%]，期望 [%]', t.proname, t.nargs, v_cmt, t.cmt;
    END IF;
    IF v_cfg <> t.cfg THEN
      RAISE EXCEPTION 'OPWRI_RB_CONFIG: %(%) proconfig 为 %，期望 %', t.proname, t.nargs, v_cfg, t.cfg;
    END IF;
    IF v_owner <> 'postgres' OR v_sec IS NOT TRUE OR v_strict IS NOT FALSE
      ...
      RAISE EXCEPTION 'OPWRI_RB_ATTRS: %(%) owner=% secdef=% strict=% parallel=% leakproof=% cost=% rows=%',
        t.proname, t.nargs, v_owner, v_sec, v_strict, v_par, v_leak, v_cost, v_rows;
    END IF;
  END LOOP;
  RAISE NOTICE 'OPWRI_RB: 6 个函数的 md5 / ACL / COMMENT / proconfig / 执行属性全部符合预期';
END
$rbpre$;
...
BEGIN
  FOR r IN SELECT p.oid, p.proname, p.pronargs FROM pg_proc p
    ...
    IF v_op <> 1 THEN
      RAISE EXCEPTION 'OPWRI_RB_CALLS: %(%) 内 operator 守卫调用 % 次，期望 1',
        r.proname, r.pronargs, v_op;
    END IF;
    IF v_ad <> 0 THEN
      RAISE EXCEPTION 'OPWRI_RB_ADMIN_CALLS: %(%) 内出现 admin 守卫调用 % 次，本批不应引入',
        r.proname, r.pronargs, v_ad;
    END IF;
  END LOOP;
  RAISE NOTICE 'OPWRI_RB: 6 个函数体内 operator 守卫调用次数均为 1，admin 守卫 0 次';
END
$calls$;

-- ===== 还原：6 个生产 canonical 逐字节 =====
...
BEGIN
  ...
  LOOP
    SELECT count(*) INTO v_n FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
     WHERE n.nspname='public' AND p.proname=t.proname AND p.pronargs=t.nargs;
    IF v_n <> 1 THEN
      RAISE EXCEPTION 'OPWRI_RB_POST_ARITY: public.%(%) 匹配到 % 个（应为 1）', t.proname, t.nargs, v_n;
    END IF;
    ...
    IF v_md5 <> t.md5 THEN
      RAISE EXCEPTION 'OPWRI_RB_POST_MD5: %(%) 定义为 %，期望 %', t.proname, t.nargs, v_md5, t.md5;
    END IF;
    IF v_acl <> t.acl THEN
      RAISE EXCEPTION 'OPWRI_RB_POST_ACL: %(%) ACL 为 %，期望 %', t.proname, t.nargs, v_acl, t.acl;
    END IF;
    IF v_cmt <> t.cmt THEN
      RAISE EXCEPTION 'OPWRI_RB_POST_COMMENT: %(%) 注释为 [%]，期望 [%]', t.proname, t.nargs, v_cmt, t.cmt;
    END IF;
    IF v_cfg <> t.cfg THEN
      RAISE EXCEPTION 'OPWRI_RB_POST_CONFIG: %(%) proconfig 为 %，期望 %', t.proname, t.nargs, v_cfg, t.cfg;
    END IF;
    IF v_owner <> 'postgres' OR v_sec IS NOT TRUE OR v_strict IS NOT FALSE
       OR v_par <> 'u' OR v_leak IS NOT FALSE OR v_cost <> 100 OR v_rows <> 1000 THEN
      RAISE EXCEPTION 'OPWRI_RB_POST_ATTRS: %(%) owner=% secdef=% strict=% parallel=% leakproof=% cost=% rows=%',
        t.proname, t.nargs, v_owner, v_sec, v_strict, v_par, v_leak, v_cost, v_rows;
    END IF;
  END LOOP;
  RAISE NOTICE 'OPWRI_RB_POST: 6 个函数的 md5 / ACL / COMMENT / proconfig / 执行属性全部符合预期';
END
$rbpost$;
...
BEGIN
  FOR r IN SELECT p.oid, p.proname, p.pronargs FROM pg_proc p
    ...
    v_ad := (length(v_def)-length(replace(v_def,'school_require_current_app_admin','')))
            /length('school_require_current_app_admin');
    IF v_op <> 0 THEN
      RAISE EXCEPTION 'OPWRI_RB_POST_CALLS: %(%) 内 operator 守卫调用 % 次，期望 0',
        r.proname, r.pronargs, v_op;
    END IF;
    IF v_ad <> 0 THEN
      RAISE EXCEPTION 'OPWRI_RB_POST_ADMIN_CALLS: %(%) 内出现 admin 守卫调用 % 次，本批不应引入',
        r.proname, r.pronargs, v_ad;
    END IF;
  END LOOP;
  RAISE NOTICE 'OPWRI_RB_POST: 6 个函数体内 operator 守卫调用次数均为 0，admin 守卫 0 次';
END
$calls$;

-- ⚠️ 回滚只还原定义与注释。
--    它【不撤销】期间已产生的任何业务记录（收入、学费账单、账户流水）。
--    事后不得把整体状态描述为「恢复到部署前」。
