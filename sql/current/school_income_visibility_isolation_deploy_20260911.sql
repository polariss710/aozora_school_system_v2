...
BEGIN
  SELECT count(*) INTO v_n FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
   WHERE n.nspname='public' AND p.proname='school_get_profit_summary_schoolwide_v1';
  IF v_n <> 1 THEN
    RAISE EXCEPTION 'IVIS_PRE_ARITY: public.school_get_profit_summary_schoolwide_v1 有 % 个（应为 1）', v_n;
  END IF;
  ...
  IF v_md5 <> 'c140ce7a34a308eae4dfb017326ada0e' THEN
    RAISE EXCEPTION 'IVIS_PRE_MD5: 定义为 %，期望 %', v_md5, 'c140ce7a34a308eae4dfb017326ada0e';
  END IF;
  IF v_acl <> '{postgres=X/postgres,authenticated=X/postgres}' THEN
    RAISE EXCEPTION 'IVIS_PRE_ACL: ACL 为 %，期望 %', v_acl, '{postgres=X/postgres,authenticated=X/postgres}';
  END IF;

  -- 角色清单的直接断言：md5 变了只说明「变了」，这里说明「变成了什么」
  IF  (v_def LIKE '%membership.role=''admin''%') THEN
    RAISE EXCEPTION 'IVIS_PRE_ROLE_SHAPE: 函数体的角色判据不是预期形状';
  END IF;
  IF NOT (v_def LIKE '%operator%' OR v_def LIKE '%read_only%') THEN
    RAISE EXCEPTION 'IVIS_PRE_ROLE_LIST: operator / read_only 的出现情况与预期不符';
  END IF;
  ...
  IF coalesce(obj_description(
    ...
    RAISE EXCEPTION 'IVIS_PRE_COMMENT: 注释被改动了（本脚本不改注释）';
  END IF;
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'IVIS_PRE_ATTRS: owner / secdef / proconfig / 执行属性与 pre 段不一致';
  END IF;

  RAISE NOTICE 'IVIS_PRE: profit RPC 的 md5 / ACL / 角色判据 / 注释 / 执行属性 全部符合预期';
END
$pre$;
...
BEGIN
  ...
  IF v_rls IS NOT TRUE OR v_force IS NOT FALSE THEN
    RAISE EXCEPTION 'IVIS_PRE_RLS: school_income_records rowsecurity=% force=%（期望 t / f）', v_rls, v_force;
  END IF;
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'IVIS_PRE_OLD_POLICY_MISSING: 找不到 school_select_operational_income_records';
  END IF;
  IF p.permissive <> 'PERMISSIVE' OR p.cmd <> 'SELECT'
    ...
    RAISE EXCEPTION 'IVIS_PRE_OLD_POLICY_DRIFT: school_select_operational_income_records 与基线不符  permissive=% cmd=% roles=% qual=%',
  ...
  IF v_n <> 0 THEN
    RAISE EXCEPTION 'IVIS_PRE_NEW_POLICY: school_restrict_part_time_work_income 有 % 条（期望 0）', v_n;
  END IF;

  SELECT count(*) INTO v_n FROM pg_policies
   WHERE schemaname='public' AND tablename='school_income_records';
  IF v_n <> 1 THEN
    RAISE EXCEPTION 'IVIS_PRE_POLICY_COUNT: school_income_records 上共有 % 条策略（期望 1）', v_n;
  END IF;

  RAISE NOTICE 'IVIS_PRE: school_income_records 的 RLS 开关与策略集合符合预期';
END
$prepol$;
...
CREATE POLICY school_restrict_part_time_work_income
  ...
  USING (
    ...
    OR EXISTS (
      SELECT 1
      FROM public.school_get_current_app_membership() membership(user_id, role, is_active)
      WHERE membership.is_active
        AND membership.role = 'admin'
    )
  );

COMMENT ON POLICY school_restrict_part_time_work_income ON public.school_income_records IS 'Hides the owner’s external part-time income from every role except an active admin. RLS only covers invoker paths; postgres and service_role have BYPASSRLS, so SECURITY DEFINER readers are deliberately unaffected.';

-- ===== B. 利润汇总 RPC：角色收紧为仅 active admin =====
...
BEGIN
  SELECT count(*) INTO v_n FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
   WHERE n.nspname='public' AND p.proname='school_get_profit_summary_schoolwide_v1';
  IF v_n <> 1 THEN
    RAISE EXCEPTION 'IVIS_POST_ARITY: public.school_get_profit_summary_schoolwide_v1 有 % 个（应为 1）', v_n;
  END IF;
  ...
  IF v_md5 <> '5eedbc896954ab48518edae8ceccae62' THEN
    RAISE EXCEPTION 'IVIS_POST_MD5: 定义为 %，期望 %', v_md5, '5eedbc896954ab48518edae8ceccae62';
  END IF;
  IF v_acl <> '{postgres=X/postgres,authenticated=X/postgres}' THEN
    RAISE EXCEPTION 'IVIS_POST_ACL: ACL 为 %，期望 %', v_acl, '{postgres=X/postgres,authenticated=X/postgres}';
  END IF;

  -- 角色清单的直接断言：md5 变了只说明「变了」，这里说明「变成了什么」
  IF NOT (v_def LIKE '%membership.role=''admin''%') THEN
    RAISE EXCEPTION 'IVIS_POST_ROLE_SHAPE: 函数体的角色判据不是预期形状';
  END IF;
  IF  (v_def LIKE '%operator%' OR v_def LIKE '%read_only%') THEN
    RAISE EXCEPTION 'IVIS_POST_ROLE_LIST: operator / read_only 的出现情况与预期不符';
  END IF;
  ...
  IF coalesce(obj_description(
       (SELECT p.oid FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
         WHERE n.nspname='public' AND p.proname='school_get_profit_summary_schoolwide_v1'),'pg_proc'),'<NULL>') <> b.b_cmt THEN
    RAISE EXCEPTION 'IVIS_POST_COMMENT: 注释被改动了（本脚本不改注释）';
  END IF;
  PERFORM 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
    WHERE n.nspname='public' AND p.proname='school_get_profit_summary_schoolwide_v1'
      AND p.proisstrict=b.b_strict AND p.proparallel=b.b_par AND p.proleakproof=b.b_leak
      ...
      AND coalesce(array_to_string(p.proconfig,', '),'<NULL>')=b.b_cfg;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'IVIS_POST_ATTRS: owner / secdef / proconfig / 执行属性与 pre 段不一致';
  END IF;

  RAISE NOTICE 'IVIS_POST: profit RPC 的 md5 / ACL / 角色判据 / 注释 / 执行属性 全部符合预期';
END
$post$;
...
BEGIN
  SELECT c.relrowsecurity, c.relforcerowsecurity INTO v_rls, v_force
    FROM pg_class c WHERE c.oid='public.school_income_records'::regclass;
  IF v_rls IS NOT TRUE OR v_force IS NOT FALSE THEN
    RAISE EXCEPTION 'IVIS_POST_RLS: school_income_records rowsecurity=% force=%（期望 t / f）', v_rls, v_force;
  END IF;
  ...
  SELECT * INTO p FROM pg_policies
   WHERE schemaname='public' AND tablename='school_income_records' AND policyname='school_select_operational_income_records';
  IF NOT FOUND THEN
    RAISE EXCEPTION 'IVIS_POST_OLD_POLICY_MISSING: 找不到 school_select_operational_income_records';
  END IF;
  IF p.permissive <> 'PERMISSIVE' OR p.cmd <> 'SELECT'
     OR p.roles::text <> '{public}' OR p.qual <> '((status <> ''incident_quarantined''::text) AND (operational_excluded IS NOT TRUE))' THEN
    RAISE EXCEPTION 'IVIS_POST_OLD_POLICY_DRIFT: school_select_operational_income_records 与基线不符  permissive=% cmd=% roles=% qual=%',
      p.permissive, p.cmd, p.roles::text, p.qual;
  END IF;
  ...
  SELECT count(*) INTO v_n FROM pg_policies
   WHERE schemaname='public' AND tablename='school_income_records' AND policyname='school_restrict_part_time_work_income';
  IF v_n <> 1 THEN
    RAISE EXCEPTION 'IVIS_POST_NEW_POLICY: school_restrict_part_time_work_income 有 % 条（期望 1）', v_n;
  END IF;

  SELECT * INTO p FROM pg_policies
   WHERE schemaname='public' AND tablename='school_income_records' AND policyname='school_restrict_part_time_work_income';
  IF p.permissive <> 'RESTRICTIVE' OR p.cmd <> 'SELECT' OR p.roles::text <> '{public}' THEN
    RAISE EXCEPTION 'IVIS_POST_NEW_POLICY_SHAPE: permissive=% cmd=% roles=%',
  ...
  WHERE (membership.is_active AND (membership.role = ''admin''::text)))))' THEN
    RAISE EXCEPTION 'IVIS_POST_NEW_POLICY_QUAL: 表达式与预期不符：%', p.qual;
  END IF;
  IF coalesce(obj_description(
    ...
    RAISE EXCEPTION 'IVIS_POST_NEW_POLICY_COMMENT: 策略注释与预期不符';
  END IF;

  SELECT count(*) INTO v_n FROM pg_policies
   WHERE schemaname='public' AND tablename='school_income_records';
  IF v_n <> 2 THEN
    RAISE EXCEPTION 'IVIS_POST_POLICY_COUNT: school_income_records 上共有 % 条策略（期望 2）', v_n;
  END IF;

  RAISE NOTICE 'IVIS_POST: school_income_records 的 RLS 开关与策略集合符合预期';
END
$postpol$;
