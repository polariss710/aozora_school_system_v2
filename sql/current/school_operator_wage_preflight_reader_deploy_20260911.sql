-- =============================================================================
-- 工资 preflight reader：admin 守卫 → operator 守卫
--
...
DO $mode$ BEGIN
  RAISE EXCEPTION 'OPWRP_MODE_INVALID: 必须指定 -v mode=rehearsal 或 -v mode=commit';
END $mode$;
\endif
...
BEGIN
  ...
  IF v_n <> 1 THEN
    RAISE EXCEPTION 'OPWRP_PRE_GUARD_MISSING: public.school_require_current_app_operator 有 % 个（应为 1）', v_n;
  END IF;
  ...
  IF v_md5 <> '526c615da0c5bf179894ccbcc325008c' THEN
    RAISE EXCEPTION 'OPWRP_PRE_GUARD_DRIFT: 守卫定义为 %，期望 %', v_md5, '526c615da0c5bf179894ccbcc325008c';
  END IF;
  IF v_acl <> '{postgres=X/postgres,authenticated=X/postgres}' THEN
    RAISE EXCEPTION 'OPWRP_PRE_GUARD_ACL: 守卫 ACL 为 %，期望 %', v_acl, '{postgres=X/postgres,authenticated=X/postgres}';
  END IF;
  RAISE NOTICE 'OPWRP_PRE: operator 守卫在位且与 2026-09-10 部署结果一致';
END
$guard$;
...
BEGIN
  ...
  IF v_n <> 1 THEN
    RAISE EXCEPTION 'OPWRP_PRE_ARITY: public.school_get_teacher_monthly_wage_generation_preflight(3) 匹配到 % 个（应为 1）', v_n;
  END IF;
  ...
  IF v_md5 <> 'b2df4d76ed9cc9533b198e247d459ac1' THEN
    RAISE EXCEPTION 'OPWRP_PRE_MD5: 定义为 %，期望 %', v_md5, 'b2df4d76ed9cc9533b198e247d459ac1';
  END IF;
  IF v_acl <> '{postgres=X/postgres,authenticated=X/postgres}' THEN
    RAISE EXCEPTION 'OPWRP_PRE_ACL: ACL 为 %，期望 %', v_acl, '{postgres=X/postgres,authenticated=X/postgres}';
  END IF;
  IF v_cmt <> 'Read-only structured wage preflight shared with the writer. candidate_prerequisites exposes the same per-lesson effective/no_wage classification for UI display; no client-side qualification rule.' THEN
    RAISE EXCEPTION 'OPWRP_PRE_COMMENT: 注释为 [%]，期望 [%]', v_cmt, 'Read-only structured wage preflight shared with the writer. candidate_prerequisites exposes the same per-lesson effective/no_wage classification for UI display; no client-side qualification rule.';
  END IF;
  IF v_cfg <> 'search_path=pg_catalog, public' THEN
    RAISE EXCEPTION 'OPWRP_PRE_CONFIG: proconfig 为 %，期望 %', v_cfg, 'search_path=pg_catalog, public';
  END IF;
  IF v_owner <> 'postgres' OR v_sec IS NOT TRUE OR v_strict IS NOT FALSE
     OR v_par <> 'u' OR v_leak IS NOT FALSE OR v_cost <> 100 THEN
    RAISE EXCEPTION 'OPWRP_PRE_ATTRS: owner=% secdef=% strict=% parallel=% leakproof=% cost=% rows=%',
  ...
  IF v_a <> 1 OR v_o <> 0 THEN
    RAISE EXCEPTION 'OPWRP_PRE_CALLS: admin 守卫 % 次（期望 1）、operator 守卫 % 次（期望 0）',
      v_a, v_o;
  END IF;

  RAISE NOTICE 'OPWRP_PRE: md5 / ACL / COMMENT / proconfig / 执行属性 / 守卫调用次数 全部符合预期';
END
$pre$;
...
BEGIN
  SELECT count(*) INTO v_n FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
   WHERE n.nspname='public' AND p.proname='school_get_teacher_monthly_wage_generation_preflight' AND p.pronargs=3;
  IF v_n <> 1 THEN
    RAISE EXCEPTION 'OPWRP_POST_ARITY: public.school_get_teacher_monthly_wage_generation_preflight(3) 匹配到 % 个（应为 1）', v_n;
  END IF;
  ...
  IF v_md5 <> '25a1f8cf76f25c8b31aa47f4c6103399' THEN
    RAISE EXCEPTION 'OPWRP_POST_MD5: 定义为 %，期望 %', v_md5, '25a1f8cf76f25c8b31aa47f4c6103399';
  END IF;
  IF v_acl <> '{postgres=X/postgres,authenticated=X/postgres}' THEN
    RAISE EXCEPTION 'OPWRP_POST_ACL: ACL 为 %，期望 %', v_acl, '{postgres=X/postgres,authenticated=X/postgres}';
  END IF;
  IF v_cmt <> 'Read-only structured wage preflight shared with the writer. candidate_prerequisites exposes the same per-lesson effective/no_wage classification for UI display; no client-side qualification rule. Requires an active admin or operator membership.' THEN
    RAISE EXCEPTION 'OPWRP_POST_COMMENT: 注释为 [%]，期望 [%]', v_cmt, 'Read-only structured wage preflight shared with the writer. candidate_prerequisites exposes the same per-lesson effective/no_wage classification for UI display; no client-side qualification rule. Requires an active admin or operator membership.';
  END IF;
  IF v_cfg <> 'search_path=pg_catalog, public' THEN
    RAISE EXCEPTION 'OPWRP_POST_CONFIG: proconfig 为 %，期望 %', v_cfg, 'search_path=pg_catalog, public';
  END IF;
  IF v_owner <> 'postgres' OR v_sec IS NOT TRUE OR v_strict IS NOT FALSE
     OR v_par <> 'u' OR v_leak IS NOT FALSE OR v_cost <> 100 THEN
    RAISE EXCEPTION 'OPWRP_POST_ATTRS: owner=% secdef=% strict=% parallel=% leakproof=% cost=% rows=%',
      v_owner, v_sec, v_strict, v_par, v_leak, v_cost, v_rows;
  END IF;
  ...
  IF v_a <> 0 OR v_o <> 1 THEN
    RAISE EXCEPTION 'OPWRP_POST_CALLS: admin 守卫 % 次（期望 0）、operator 守卫 % 次（期望 1）',
      v_a, v_o;
  END IF;

  RAISE NOTICE 'OPWRP_POST: md5 / ACL / COMMENT / proconfig / 执行属性 / 守卫调用次数 全部符合预期';
END
$post$;
