...
--   ⚠️ **六个函数的实际定义**一旦变动，这六个值即失效，须重新 rehearsal 取值并重审。
--      只改注释或文档不影响它们。
...
--
--   ⚠️ **六个函数的实际定义**一旦变动，这六个值即失效，须由新的 rehearsal 重取并重审。
--      只改部署脚本的注释或文档不影响它们——canonical 文本没变，md5 就没变。
...
BEGIN
  FOR r IN
    SELECT e.fn_key, e.expect_md5, t.signature
  ...
  %s = %L', r.fn_key, r.expect_md5);
    ...
    END IF;
    v_cur := md5(pg_get_functiondef(to_regprocedure(r.signature)::oid));
    IF v_cur IS DISTINCT FROM r.expect_md5 THEN
      v_bad := v_bad || format(E'
  %s
    认可的 v2 基线 %s
    生产当前       %s',
                               r.signature, r.expect_md5, v_cur);
    END IF;
  END LOOP;
  IF v_missing <> '' THEN
...
--   注意执行顺序：A-0 不通过会直接 RAISE，**走不到这里**。所以这一层不是
--   「md5 变了之后的定位手段」——那条路径根本不会发生。
--   它的实际作用是：在 A-0 通过（定义与内嵌基线逐字节相同）之后，
--   再独立确认那份定义确实带着 v2 的结构特征，
--   以防内嵌常量本身被改成了别的东西的 md5。
-- ---------------------------------------------------------------------------
DO $do$
