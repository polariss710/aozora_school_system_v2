-- P0-D blocker classification matrix. Requires the fixed synthetic fixture.
...
declare
  ...
  v_ids constant uuid[]:=array[
    'd0d00000-0000-4000-8000-000000008101'::uuid,
    'd0d00000-0000-4000-8000-000000008102'::uuid,
    'd0d00000-0000-4000-8000-000000008103'::uuid,
    'd0d00000-0000-4000-8000-000000008104'::uuid
  ];
begin
  ...
  begin
    ...
    if v_code<>'TUITION_VOID_INCOME_NOT_PENDING' then
      raise exception 'P0D_RECEIVED_BLOCKER_INVALID: %',v_code;
    end if;
    raise exception 'P0D_EXPECTED_SUBTX_ROLLBACK';
  exception when others then
    if sqlerrm<>'P0D_EXPECTED_SUBTX_ROLLBACK' then raise; end if;
  end;
  ...
  for v_i in 1..cardinality(v_statuses) loop
    ...
    begin
      insert into public.school_personal_cash_income_linkage_events(
        ...
        'd0d00000-0000-4000-8000-000000009001','d0d00000-0000-4000-8000-000000009002',
        'codex-test P0-D cash account','codex-test','home_jpy_transactions',
        case when v_status='synced' then 'd0d00000-0000-4000-8000-000000009104'::uuid end,
        'JPY',(select amount from public.school_income_records where id=v_income),
        'codex-test-p0d-'||v_status,v_status,'codex-test P0-D blocker',
        'JPY',(select amount from public.school_income_records where id=v_income),
        case when v_status in ('awaiting_cash_confirmation','cash_rejected','synced')
          then ('d0d00000-0000-4000-8000-0000000091'||lpad(v_i::text,2,'0'))::uuid end,
      ...
      if v_code<>'TUITION_VOID_CASH_FACT_EXISTS' then
        raise exception 'P0D_CASH_BLOCKER_INVALID[%]: %',v_status,v_code;
      end if;
      raise exception 'P0D_EXPECTED_SUBTX_ROLLBACK';
    exception when others then
      if sqlerrm<>'P0D_EXPECTED_SUBTX_ROLLBACK' then raise; end if;
  ...
  begin
    perform * from public.school_lock_student_monthly_settlement(
      v_student,'2020-08','codex-test P0-D settlement blocker');
    select blocker_code into strict v_code
    from public.school_get_atomic_tuition_void_preflight(v_income);
    if v_code<>'TUITION_VOID_DOWNSTREAM_FACT_EXISTS' then
      raise exception 'P0D_SETTLEMENT_BLOCKER_INVALID: %',v_code;
    end if;
    raise exception 'P0D_EXPECTED_SUBTX_ROLLBACK';
  exception when others then
    if sqlerrm<>'P0D_EXPECTED_SUBTX_ROLLBACK' then raise; end if;
  ...
  begin
    ...
    raise exception 'P0D_MANIFEST_MISMATCH_NOT_REJECTED';
  exception when others then
    if position('TUITION_VOID_MANIFEST_MISMATCH' in sqlerrm)=0 then raise; end if;
  end;
  begin
    perform * from public.school_void_atomic_student_tuition_generation_local(
      'd0d00000-0000-4000-8000-000000004099',v_bill,v_income,v_manifest,
      'codex-test non-active revision');
    raise exception 'P0D_NON_ACTIVE_NOT_REJECTED';
  exception when others then
  ...
  if v_code<>'TUITION_VOID_NOT_ATOMIC' then
    raise exception 'P0D_HISTORICAL_BLOCKER_INVALID: %',v_code;
  end if;
  ...
  begin
    ...
    if v_code<>'TUITION_VOID_MANIFEST_MISMATCH' then
      raise exception 'P0D_INCOMPLETE_CHAIN_BLOCKER_INVALID: %',v_code;
    end if;
    raise exception 'P0D_EXPECTED_SUBTX_ROLLBACK';
  exception when others then
    if sqlerrm<>'P0D_EXPECTED_SUBTX_ROLLBACK' then raise; end if;
...
rollback;
\echo 'P0D_BLOCKER_MATRIX_ROLLED_BACK'
