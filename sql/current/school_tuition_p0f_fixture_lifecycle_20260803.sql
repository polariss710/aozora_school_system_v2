-- P0-F committed whitelist fixture lifecycle.
-- Required: -v p0f_fixture_action=preflight|insert|verify|cleanup|residue
\set ON_ERROR_STOP on
\pset pager off
\if :{?p0f_fixture_action}
\else
  \echo 'P0F_FIXTURE_ACTION_REQUIRED'
  \quit
\endif
begin;
set local lock_timeout='10s';
set local statement_timeout='240s';
select set_config('school.p0f_fixture_action',:'p0f_fixture_action',true);

do $fixture$
declare
  v_action text:=current_setting('school.p0f_fixture_action');
  v_marker constant text:='codex-test tuition-p0f-commit-20260803';
  v_entity constant uuid:='2cf7b72f-6e3c-4d09-80f7-7c58593cd466';
  v_subject constant uuid:='f0f10000-0000-4000-8000-00000000d001';
  v_teacher constant uuid:='f0f10000-0000-4000-8000-000000007001';
  v_student constant uuid:='f0f10000-0000-4000-8000-00000000a001';
  v_lesson constant uuid:='f0f10000-0000-4000-8000-000000001001';
  v_income constant uuid:='f0f10000-0000-4000-8000-000000007101';
  v_preview record;
begin
  if v_action not in ('preflight','insert','verify','cleanup','residue') then
    raise exception 'P0F_FIXTURE_ACTION_INVALID';
  end if;
  if v_action in ('preflight','residue') then
    if exists(
      select 1 from public.school_subjects where id=v_subject or note=v_marker
      ...
      union all select 1 from public.school_student_settlement_source_treatment_drafts
        where student_id=v_student
    ) then raise exception 'P0F_FIXTURE_RESIDUE'; end if;
  elsif v_action='insert' then
    if exists(select 1 from public.school_students where id=v_student)
       or exists(select 1 from public.school_lesson_records where id=v_lesson) then
      raise exception 'P0F_FIXTURE_COLLISION';
    end if;
    insert into public.school_subjects(id,name,category,is_active,note,primary_category)
    values(v_subject,'codex-test P0-F commit subject','codex-test',true,v_marker,'班课');
    ...
    values(v_teacher,'codex-test-p0f-commit','codex-test P0-F commit teacher',
      'codex-test P0-F commit teacher',v_subject,v_entity,'active',v_marker,'school');
    ...
    values(v_student,'codex-test-p0f-commit','codex-test P0-F commit student',
      'codex-test P0-F commit student',v_entity,'active','school',0.05,0,v_marker);
    ...
    ) values(v_lesson,'planned','2036-07-09','2036-07',v_student,v_teacher,v_subject,
      v_entity,'10:00','12:00',2,v_marker,'pending_makeup',true,v_marker,'school',
      8500,17000,1,120,'online',v_marker,'2036-07','2036-07-07','2036-07-09',
      '2036-07','explicit_billing_week_at_create',now());
    ...
    ) values(v_income,v_entity,v_student,'2036-07-20','2036-07','2036-07','tuition',
      v_marker,'JPY',17000,17000,'JPY','received',false,'codex-test',true,v_marker,'school',false);
    perform * from public.school_set_student_settlement_source_treatment_draft(
      v_student,'2036-07','net_lesson_variance_to_financial_credit_v1',0.042,
      'codex_test_commit_rate_v1','2036-07-01',v_marker
    );
  elsif v_action='verify' then
    select * into strict v_preview
    from public.school_preview_student_settlement_source_treatment(
      v_student,'2036-07',null,null,null,null
    );
    if v_preview.pending_makeup_hours<>2
       or v_preview.unused_planned_credit_jpy<>-17000
       or v_preview.net_lesson_variance_cny<>-714
       or v_preview.system_difference_cny<>-714
       or v_preview.lesson_variance_source_count<>1
       or (select count(*) from public.school_student_settlement_source_treatment_drafts
           where student_id=v_student and status='active')<>1 then
      raise exception 'P0F_FIXTURE_VERIFY_FAILED: %',to_jsonb(v_preview);
    end if;
    ...
    if (select count(*) from public.school_students where id=v_student and note=v_marker)<>1
       ...
       or (select count(*) from public.school_student_settlement_source_treatment_drafts
           where student_id=v_student and reason=v_marker)<>1 then
      raise exception 'P0F_FIXTURE_CLEANUP_OWNERSHIP_FAILED';
    end if;
    ...
    delete from public.school_lesson_records where id=v_lesson and note=v_marker;
