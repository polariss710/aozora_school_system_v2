...
-- ⚠️ 回滚不依赖 admin 守卫（还原后的定义不引用它）⇒ 守卫异常只告警。
...
BEGIN
  ...
  IF v_n <> 1 THEN
    RAISE WARNING 'ACCT_RB_GUARD_ARITY: public.school_require_current_app_admin 有 % 个（应为 1）', v_n;
    RETURN;
  END IF;
  ...
  IF v_md5 <> 'f9e6563d875bbb1bc61be31531da3d69' THEN
    RAISE WARNING 'ACCT_RB_GUARD_DRIFT: 守卫定义为 %，期望 %', v_md5, 'f9e6563d875bbb1bc61be31531da3d69';
    RETURN;
  END IF;
  IF v_acl <> '{postgres=X/postgres,authenticated=X/postgres}' THEN
    RAISE WARNING 'ACCT_RB_GUARD_ACL: 守卫 ACL 为 %，期望 %', v_acl, '{postgres=X/postgres,authenticated=X/postgres}';
    RETURN;
  END IF;
  RAISE NOTICE 'ACCT_RB: admin 守卫状态正常（回滚本身不依赖它）';
END
$g$;
...
BEGIN
  FOR t IN SELECT * FROM (VALUES
    ...
    IF v_n <> 1 THEN
      RAISE EXCEPTION 'ACCT_RB_ARITY: %有 % 个（应为 1）', t.proname, v_n;
    END IF;
    ...
    IF v_md5 <> t.md5 THEN
      RAISE EXCEPTION 'ACCT_RB_MD5: % 定义为 %，期望 %', t.proname, v_md5, t.md5;
    END IF;
    IF v_acl <> '{postgres=X/postgres,authenticated=X/postgres,service_role=X/postgres}' THEN
      RAISE EXCEPTION 'ACCT_RB_ACL: % 的 ACL 为 %，期望 %', t.proname, v_acl, '{postgres=X/postgres,authenticated=X/postgres,service_role=X/postgres}';
    END IF;

    v_g := (length(v_def)-length(replace(v_def,'school_require_current_app_admin','')))/length('school_require_current_app_admin');
    IF v_g <> 1 THEN
      RAISE EXCEPTION 'ACCT_RB_GUARD_CALLS: % 内 admin 守卫 % 次（期望 1）', t.proname, v_g;
    END IF;
    ...
    IF coalesce(obj_description(
      ...
      RAISE EXCEPTION 'ACCT_RB_COMMENT: % 的注释与取证基线不符', t.proname;
    END IF;

    SELECT * INTO b FROM acct_before WHERE b_name = t.proname;
    IF NOT FOUND THEN
      RAISE EXCEPTION 'ACCT_RB_CAPTURE_MISSING: pre 段没记下 %', t.proname;
    END IF;
    ...
    IF NOT FOUND THEN
      RAISE EXCEPTION 'ACCT_RB_ATTRS: % 的注释 / owner / secdef / proconfig / 执行属性与 pre 段不一致', t.proname;
    END IF;
  END LOOP;
  RAISE NOTICE 'ACCT_RB: 4 个函数的 md5 / ACL / 守卫调用 / 注释 / 执行属性 全部符合预期';
END
$rbpre$;
...
BEGIN
  FOR t IN SELECT unnest(ARRAY['school_account_adjustments','school_account_transfers']) AS tbl LOOP
    ...
    IF v_acl <> '{postgres=arwdDxtm/postgres,authenticated=rxtm/postgres,service_role=arwdDxtm/postgres}' THEN
      RAISE EXCEPTION 'ACCT_RB_TBL_ACL: % 的 ACL 为 %，期望 %', t.tbl, v_acl, '{postgres=arwdDxtm/postgres,authenticated=rxtm/postgres,service_role=arwdDxtm/postgres}';
    END IF;
    -- 本脚本【不动】RLS 开关，前后都应是 false/false
    IF v_rls IS NOT FALSE OR v_force IS NOT FALSE THEN
      RAISE EXCEPTION 'ACCT_RB_TBL_RLS: % rowsecurity=% force=%（本批不改，期望 f/f）', t.tbl, v_rls, v_force;
    END IF;
    SELECT count(*) INTO v_pol FROM pg_policies
     WHERE schemaname='public' AND tablename=t.tbl;
    IF v_pol <> 0 THEN
      RAISE EXCEPTION 'ACCT_RB_TBL_POLICY: % 上有 % 条策略（期望 0）', t.tbl, v_pol;
    END IF;
  END LOOP;
  RAISE NOTICE 'ACCT_RB: 两张表的 ACL / RLS 开关 / 策略数 全部符合预期';
END
$rbpretbl$;

-- ===== A. 还原 authenticated 的写权限 =====
GRANT INSERT, UPDATE, DELETE, TRUNCATE ON TABLE public.school_account_adjustments TO authenticated;
GRANT INSERT, UPDATE, DELETE, TRUNCATE ON TABLE public.school_account_transfers TO authenticated;
...
BEGIN
  FOR t IN SELECT * FROM (VALUES
    ...
    SELECT count(*) INTO v_n FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
     WHERE n.nspname='public' AND p.proname=t.proname;
    IF v_n <> 1 THEN
      RAISE EXCEPTION 'ACCT_RB_POST_ARITY: %有 % 个（应为 1）', t.proname, v_n;
    END IF;
    ...
    IF v_md5 <> t.md5 THEN
      RAISE EXCEPTION 'ACCT_RB_POST_MD5: % 定义为 %，期望 %', t.proname, v_md5, t.md5;
    END IF;
    IF v_acl <> '{postgres=X/postgres,authenticated=X/postgres,service_role=X/postgres}' THEN
      RAISE EXCEPTION 'ACCT_RB_POST_ACL: % 的 ACL 为 %，期望 %', t.proname, v_acl, '{postgres=X/postgres,authenticated=X/postgres,service_role=X/postgres}';
    END IF;

    v_g := (length(v_def)-length(replace(v_def,'school_require_current_app_admin','')))/length('school_require_current_app_admin');
    IF v_g <> 0 THEN
      RAISE EXCEPTION 'ACCT_RB_POST_GUARD_CALLS: % 内 admin 守卫 % 次（期望 0）', t.proname, v_g;
    END IF;
    ...
    IF coalesce(obj_description(
         (SELECT p.oid FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
           WHERE n.nspname='public' AND p.proname=t.proname),'pg_proc'),'<NULL>') <> t.cmt THEN
      RAISE EXCEPTION 'ACCT_RB_POST_COMMENT: % 的注释与取证基线不符', t.proname;
    END IF;

    SELECT * INTO b FROM acct_before WHERE b_name = t.proname;
    IF NOT FOUND THEN
      RAISE EXCEPTION 'ACCT_RB_POST_CAPTURE_MISSING: pre 段没记下 %', t.proname;
    END IF;
    PERFORM 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
      WHERE n.nspname='public' AND p.proname=t.proname
        AND coalesce(obj_description(p.oid,'pg_proc'),'<NULL>')=b.b_cmt
        ...
        AND coalesce(array_to_string(p.proconfig,', '),'<NULL>')=b.b_cfg;
    IF NOT FOUND THEN
      RAISE EXCEPTION 'ACCT_RB_POST_ATTRS: % 的注释 / owner / secdef / proconfig / 执行属性与 pre 段不一致', t.proname;
    END IF;
  END LOOP;
  RAISE NOTICE 'ACCT_RB_POST: 4 个函数的 md5 / ACL / 守卫调用 / 注释 / 执行属性 全部符合预期';
END
$rbpost$;
...
BEGIN
  FOR t IN SELECT unnest(ARRAY['school_account_adjustments','school_account_transfers']) AS tbl LOOP
    ...
    IF v_acl <> '{postgres=arwdDxtm/postgres,authenticated=arwdDxtm/postgres,service_role=arwdDxtm/postgres}' THEN
      RAISE EXCEPTION 'ACCT_RB_POST_TBL_ACL: % 的 ACL 为 %，期望 %', t.tbl, v_acl, '{postgres=arwdDxtm/postgres,authenticated=arwdDxtm/postgres,service_role=arwdDxtm/postgres}';
    END IF;
    -- 本脚本【不动】RLS 开关，前后都应是 false/false
    IF v_rls IS NOT FALSE OR v_force IS NOT FALSE THEN
      RAISE EXCEPTION 'ACCT_RB_POST_TBL_RLS: % rowsecurity=% force=%（本批不改，期望 f/f）', t.tbl, v_rls, v_force;
    END IF;
    SELECT count(*) INTO v_pol FROM pg_policies
     WHERE schemaname='public' AND tablename=t.tbl;
    IF v_pol <> 0 THEN
      RAISE EXCEPTION 'ACCT_RB_POST_TBL_POLICY: % 上有 % 条策略（期望 0）', t.tbl, v_pol;
    END IF;
  END LOOP;
  RAISE NOTICE 'ACCT_RB_POST: 两张表的 ACL / RLS 开关 / 策略数 全部符合预期';
END
$rbposttbl$;
