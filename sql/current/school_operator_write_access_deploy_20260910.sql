-- =============================================================================
-- 教务老师写入权限：工资与支出（第一批 · 开放）
--
-- 业务划分（业务负责人 2026-09-10 定）：
--   **她做财务（在账内产生凭证），他做出纳（资金实际进出）。**
--
-- 本批把 8 个 writer 的守卫由「仅 admin」改为「admin 或 operator」：
--   工资：生成快照(两个重载) / 调明细 / 生成支付请求 / 作废未提交的工资支出记录
--   支出：创建 / 修改 / 建 Cash 待提交记录
--
-- ⛔ 【不动】（仍仅 admin）：
--     school_void_teacher_wage_lock      工资快照作废
--     school_reverse_expense_record      支出冲销
--     两个 Cash 提交 Edge —— 它们先用【原用户 JWT】调 admin 守卫再切 service_role，
--     「operator 不得提交 Cash」由此守住，本次不碰。
--
-- ⛔ 【不在本批】：收入六个入口（那是【收敛】——它们现在没有任何角色守卫，
--     谁登录都能调，含冲销）；月度结算草稿（需同时改 Edge 与库内 online
--     包装层，单独一轮）。
--
-- 改动形态
--   1. 新建 school_require_current_app_operator()，照
--      school_require_current_app_admin 的形状写：同样读 auth.uid()、同样要求
--      is_active、同样的 42501 与 P0G1_ 错误码风格、同样的 ACL。
--   2. 8 个函数各替换【恰好一处】守卫调用（补丁生成时对此有断言）。
--      签名未变 ⇒ CREATE OR REPLACE，不 DROP ⇒ ACL 与 COMMENT 自动保留，
--      不存在默认授权把 service_role 放回数组的问题。
--   3. ⚠️ 三个函数的 COMMENT 明写 Active-admin-only，一并改。
--      只换守卫不改注释，注释就成了假话 —— 下一个读它的人会据此判断权限。
--
-- 基线：Codex 2026-09-10 15:15 生产取证（opcanon/）。
--   ⚠️ 8 个 COMMENT 全部非 NULL 并逐条钉住。前一天正因把 COMMENT 记成 NULL，
--      害生产 rehearsal 白跑一轮。
--   渲染口径先验证过：8 个 canonical 装进本地 harness 后 8/8 复现生产 md5，
--   再在其上打补丁算部署后 md5。
--
-- 用法
...
BEGIN;
SELECT set_config('opw.mode', :'mode', false);

DO $gate$
BEGIN
  IF current_setting('opw.mode') NOT IN ('rehearsal','commit') THEN
    RAISE EXCEPTION 'OPW_MODE_REQUIRED: 需要 -v mode=rehearsal 或 -v mode=commit，得到 %',
      current_setting('opw.mode');
  END IF;
END
$gate$;

CREATE TEMP TABLE opw_target(
  proname text, nargs int, pre_md5 text, post_md5 text,
  acl text, pre_cmt text, post_cmt text) ON COMMIT DROP;
INSERT INTO opw_target VALUES
...
DO $pre$
DECLARE
  r record; v_oid oid; v_n int; v_md5 text; v_acl text; v_cfg text; v_cmt text;
...
BEGIN
  ...
  IF v_n <> 0 THEN
    RAISE EXCEPTION 'OPW_PRE_GUARD_EXISTS: school_require_current_app_operator 已有 % 个，'
      '本脚本负责创建它，拒绝覆盖未知实现', v_n;
  END IF;

  -- 新守卫是照 admin 守卫写的；它变了就要重新比对，不能照跑。
  ...
  IF v_md5 IS DISTINCT FROM 'f9e6563d875bbb1bc61be31531da3d69' THEN
    RAISE EXCEPTION 'OPW_PRE_ADMIN_GUARD_DRIFT: admin 守卫 md5=%', coalesce(v_md5,'<不存在>');
  END IF;

  FOR r IN SELECT * FROM opw_target ORDER BY proname, nargs LOOP
    SELECT count(*) INTO v_n FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
     WHERE n.nspname='public' AND p.proname=r.proname AND p.pronargs=r.nargs;
    IF v_n <> 1 THEN
      RAISE EXCEPTION 'OPW_PRE_NOT_UNIQUE: %(% 参数) 匹配 % 个，应为 1', r.proname, r.nargs, v_n; END IF;
    ...
    SELECT p.oid, md5(pg_get_functiondef(p.oid)), coalesce(p.proacl::text,''),
      ...
      INTO v_oid, v_md5, v_acl, v_cfg, v_cmt, v_own, v_sec, v_strict, v_par, v_leak, v_cost, v_rows
      FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
     WHERE n.nspname='public' AND p.proname=r.proname AND p.pronargs=r.nargs;

    IF v_md5 <> r.pre_md5 THEN
      RAISE EXCEPTION 'OPW_PRE_MD5: %(%) 得 %  期望 %', r.proname, r.nargs, v_md5, r.pre_md5; END IF;
    IF v_acl <> r.acl THEN
      RAISE EXCEPTION 'OPW_PRE_ACL: %(%) 得 %  期望 %', r.proname, r.nargs, v_acl, r.acl; END IF;
    IF v_cfg <> 'search_path=pg_catalog, public' THEN
      RAISE EXCEPTION 'OPW_PRE_PROCONFIG: %(%) 得 %', r.proname, r.nargs, v_cfg; END IF;
    IF v_cmt IS DISTINCT FROM r.pre_cmt THEN
      RAISE EXCEPTION 'OPW_PRE_COMMENT: %(%) 注释与取证不符%实际: %',
        r.proname, r.nargs, chr(10), coalesce(v_cmt,'<null>'); END IF;
    IF v_own <> 'postgres' OR v_sec IS NOT TRUE OR v_strict IS NOT FALSE
      ...
      RAISE EXCEPTION 'OPW_PRE_ATTRS: %(%) owner=% secdef=% strict=% parallel=% leak=% cost=% rows=%',
        r.proname, r.nargs, v_own, v_sec, v_strict, v_par, v_leak, v_cost, v_rows; END IF;
  END LOOP;
  RAISE NOTICE 'OPW: 部署前基线通过（8 个函数 + admin 守卫未漂移 + 新守卫尚不存在）';
END
$pre$;
...
-- §2 新建 operator 守卫
-- -----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.school_require_current_app_operator()
 RETURNS uuid
 LANGUAGE plpgsql
...
begin
  ...
  if not exists (
    ...
    raise exception using
      errcode='42501',
      message='P0G1_ACTIVE_OPERATOR_REQUIRED';
  end if;
...
-- ⚠️ 逐条 GRANT。一条 GRANT 带多个角色会让 ACL 数组顺序随授权顺序变化 ——
--    这两天已因此失败过四次。这里只授 authenticated，与 admin 守卫一致
--    （admin 守卫的 service_role 也没有 EXECUTE）。
REVOKE ALL ON FUNCTION public.school_require_current_app_operator() FROM PUBLIC;
REVOKE ALL ON FUNCTION public.school_require_current_app_operator() FROM anon;
REVOKE ALL ON FUNCTION public.school_require_current_app_operator() FROM authenticated;
REVOKE ALL ON FUNCTION public.school_require_current_app_operator() FROM service_role;
GRANT EXECUTE ON FUNCTION public.school_require_current_app_operator() TO authenticated;
COMMENT ON FUNCTION public.school_require_current_app_operator() IS 'P0-G1 active admin-or-operator assertion for the current authenticated auth.uid; callable only by authenticated user-scoped School clients.';

-- -----------------------------------------------------------------------------
-- §3 替换 8 个 writer 的守卫调用（签名不变 ⇒ CREATE OR REPLACE）
-- -----------------------------------------------------------------------------
...
begin
  v_actor := public.school_require_current_app_operator();
...
-- -----------------------------------------------------------------------------
-- §4 COMMENT：三个函数写着 Active-admin-only，一并改
--    签名【动态解析】，不写死参数类型 —— 抄一遍类型表只是多一处出错的机会。
-- -----------------------------------------------------------------------------
DO $cmt$
DECLARE r record; v_sig text; v_n int := 0;
BEGIN
  FOR r IN SELECT * FROM opw_target WHERE post_cmt IS DISTINCT FROM pre_cmt LOOP
    SELECT 'public.'||quote_ident(p.proname)||'('||pg_get_function_identity_arguments(p.oid)||')'
     ...
     WHERE n.nspname='public' AND p.proname=r.proname AND p.pronargs=r.nargs;
    EXECUTE format('COMMENT ON FUNCTION %s IS %L', v_sig, r.post_cmt);
    v_n := v_n + 1;
  END LOOP;
  IF v_n <> 3 THEN
    RAISE EXCEPTION 'OPW_COMMENT_COUNT: 改了 % 条注释，应为 3', v_n; END IF;
  RAISE NOTICE 'OPW: 3 条 Active-admin-only 注释已改为 Active admin or operator';
END
$cmt$;
...
DECLARE
  ...
  v_cost real; v_rows real;
BEGIN
  -- 新守卫：唯一、定义、ACL、COMMENT
  SELECT count(*) INTO v_n FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
   WHERE n.nspname='public' AND p.proname='school_require_current_app_operator';
  IF v_n <> 1 THEN
    RAISE EXCEPTION 'OPW_POST_GUARD_COUNT: 新守卫有 % 个（应为 1）', v_n; END IF;
  SELECT p.oid, md5(pg_get_functiondef(p.oid)), coalesce(p.proacl::text,''),
         obj_description(p.oid,'pg_proc'), pg_get_userbyid(p.proowner), p.prosecdef,
         coalesce(array_to_string(p.proconfig,','),'')
    INTO v_oid, v_md5, v_acl, v_cmt, v_own, v_sec, v_cfg
    FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
   WHERE n.nspname='public' AND p.proname='school_require_current_app_operator';
  IF v_md5 <> '526c615da0c5bf179894ccbcc325008c' THEN
    RAISE EXCEPTION 'OPW_POST_GUARD_MD5: 得 %', v_md5; END IF;
  IF v_acl <> '{postgres=X/postgres,authenticated=X/postgres}' THEN
    RAISE EXCEPTION 'OPW_POST_GUARD_ACL: 得 %  期望与 admin 守卫一致', v_acl; END IF;
  IF v_own <> 'postgres' OR v_sec IS NOT TRUE
     OR v_cfg <> 'search_path=pg_catalog, public' THEN
    RAISE EXCEPTION 'OPW_POST_GUARD_ATTRS: owner=% secdef=% cfg=%', v_own, v_sec, v_cfg; END IF;
  IF v_cmt IS NULL THEN RAISE EXCEPTION 'OPW_POST_GUARD_NO_COMMENT'; END IF;
  -- service_role 不该有 —— admin 守卫也没有。PUBLIC 用 aclexplode 单独查，
  -- has_function_privilege 查不出 PUBLIC 本身。
  IF EXISTS (SELECT 1 FROM aclexplode(coalesce(
               (SELECT p2.proacl FROM pg_proc p2 WHERE p2.oid=v_oid),
               acldefault('f',(SELECT p2.proowner FROM pg_proc p2 WHERE p2.oid=v_oid)))) a
              WHERE a.grantee = 0 AND a.privilege_type='EXECUTE') THEN
    RAISE EXCEPTION 'OPW_POST_GUARD_PUBLIC: 新守卫对 PUBLIC 开放'; END IF;
  IF has_function_privilege('anon', v_oid, 'EXECUTE') THEN
    RAISE EXCEPTION 'OPW_POST_GUARD_ANON: 新守卫对 anon 开放'; END IF;
  ...
  FOR r IN SELECT * FROM opw_target ORDER BY proname, nargs LOOP
    SELECT count(*) INTO v_n FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
     WHERE n.nspname='public' AND p.proname=r.proname AND p.pronargs=r.nargs;
    IF v_n <> 1 THEN
      RAISE EXCEPTION 'OPW_POST_NOT_UNIQUE: %(%) 匹配 % 个', r.proname, r.nargs, v_n; END IF;
    ...
    IF v_md5 <> r.post_md5 THEN
      RAISE EXCEPTION 'OPW_POST_MD5: %(%) 得 %  期望 %', r.proname, r.nargs, v_md5, r.post_md5; END IF;
    -- 签名未变 ⇒ CREATE OR REPLACE 不该动 ACL。变了说明发生了预期外的重建。
    IF v_acl <> r.acl THEN
      RAISE EXCEPTION 'OPW_POST_ACL: %(%) 得 %  ← 权限被改动', r.proname, r.nargs, v_acl; END IF;
    IF v_cfg <> 'search_path=pg_catalog, public' THEN
      RAISE EXCEPTION 'OPW_POST_PROCONFIG: %(%) 得 %', r.proname, r.nargs, v_cfg; END IF;
    IF v_cmt IS DISTINCT FROM r.post_cmt THEN
      RAISE EXCEPTION 'OPW_POST_COMMENT: %(%) 注释不符%实际: %',
        r.proname, r.nargs, chr(10), coalesce(v_cmt,'<null>'); END IF;
    IF v_own <> 'postgres' OR v_sec IS NOT TRUE OR v_strict IS NOT FALSE
       OR v_par <> 'u' OR v_leak IS NOT FALSE OR v_cost <> 100 OR v_rows <> 1000 THEN
      RAISE EXCEPTION 'OPW_POST_ATTRS: %(%) owner=% secdef=% strict=% parallel=% leak=% cost=% rows=%',
        r.proname, r.nargs, v_own, v_sec, v_strict, v_par, v_leak, v_cost, v_rows; END IF;
  END LOOP;

  -- 守卫落点：8 个函数体里都不该再残留 admin 调用，且都该调到 operator。
  --
  -- ⚠️ 这段【必须逐个查】，不能写成一条带 pg_get_functiondef 的聚合查询：
  --    规划器会把 WHERE 里的 pg_get_functiondef 提前到 pg_proc 全表扫描上求值，
  --    在命名空间过滤生效之前就碰到聚合函数，报
  --    「"array_agg" is an aggregate function」。harness 上实测过。
  v_n := 0;
  FOR r IN SELECT * FROM opw_target ORDER BY proname, nargs LOOP
    SELECT pg_get_functiondef(p.oid) INTO v_cmt
      FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
     WHERE n.nspname='public' AND p.proname=r.proname AND p.pronargs=r.nargs;
    IF position('school_require_current_app_admin' in v_cmt) > 0 THEN
      RAISE EXCEPTION 'OPW_POST_ADMIN_GUARD_REMAINS: %(%) 函数体里仍有 admin 守卫调用',
        r.proname, r.nargs; END IF;
    IF position('school_require_current_app_operator' in v_cmt) = 0 THEN
      RAISE EXCEPTION 'OPW_POST_OPERATOR_GUARD_MISSING: %(%) 没有调到 operator 守卫',
        r.proname, r.nargs; END IF;
    v_n := v_n + 1;
  END LOOP;
  IF v_n <> 8 THEN
    RAISE EXCEPTION 'OPW_POST_GUARD_SCAN_COUNT: 只扫到 % 个函数（应为 8）', v_n; END IF;

  RAISE NOTICE 'OPW: 部署后断言全部通过（新守卫 + 8 个函数的定义/ACL/注释/属性/守卫落点）';
END
$post$;
...
-- §6 身份探针：直接调新守卫
--
--   新守卫是 STABLE 的纯读函数，只查 school_app_memberships ⇒ 【零写入】，
--   可以在生产上放心探。这一点与顺序守卫那次不同 —— 那次的放行路径要调真实
--   writer，才不得不留成未验证。
--
--   ⚠️ 生产只有 admin 与 operator 两种真身份。「停用的 operator」与
--      「read_only」造不出对象来验，只能在 harness 验（已验）。
--      本节覆盖四种：真 admin、真 operator、有 uid 但无成员、未登录。
-- -----------------------------------------------------------------------------
DO $probe$
DECLARE
  v_admin uuid; v_operator uuid; v_err text; v_state text; v_got text; v_bad int := 0;
  r record;
BEGIN
  SELECT user_id INTO v_admin FROM public.school_app_memberships
   WHERE role='admin' AND is_active ORDER BY user_id LIMIT 1;
  SELECT user_id INTO v_operator FROM public.school_app_memberships
   WHERE role='operator' AND is_active ORDER BY user_id LIMIT 1;
  IF v_admin IS NULL OR v_operator IS NULL THEN
    RAISE EXCEPTION 'OPW_PROBE_NO_IDENTITY: 库里找不到有效的 admin 或 operator，无法探';
  END IF;

  FOR r IN SELECT * FROM (VALUES
    (v_admin::text,    'pass', '真实 active admin'),
    (v_operator::text, 'pass', '真实 active operator'),
    ('99999999-9999-4999-8999-999999999999', 'norole', '有 uid 但无成员记录'),
    ('',               'noauth','未登录')
  ) AS t(sub, expect, note) LOOP
    PERFORM set_config('request.jwt.claim.sub', r.sub, true);
    PERFORM set_config('request.jwt.claims',
      CASE WHEN r.sub = '' THEN '' ELSE json_build_object('sub', r.sub)::text END, true);
    BEGIN
      PERFORM public.school_require_current_app_operator();
      v_state := '<none>'; v_err := '<无异常>';
    EXCEPTION WHEN OTHERS THEN
      v_state := SQLSTATE; v_err := SQLERRM;
    END;

    -- 只认【精确】结果。没分类的失败不得算成通过 —— 那正是这个探针要防的。
    v_got := CASE
      WHEN v_state = '<none>' THEN 'pass'
      WHEN v_state = '42501' AND position('P0G1_ACTIVE_OPERATOR_REQUIRED' in v_err) > 0 THEN 'norole'
      WHEN v_state = '42501' AND position('P0G1_AUTH_REQUIRED' in v_err) > 0 THEN 'noauth'
      ELSE 'unclassified' END;

    IF v_got <> r.expect THEN
      v_bad := v_bad + 1;
      RAISE WARNING 'OPW_PROBE_MISMATCH: %（%）期望 % 实得 % [sqlstate %] —— %',
        r.note, r.sub, r.expect, v_got, v_state, v_err;
    END IF;
  END LOOP;
  ...
  IF v_bad > 0 THEN
    RAISE EXCEPTION 'OPW_PROBE: % 组与预期不符，见上方 WARNING', v_bad; END IF;
  RAISE NOTICE 'OPW: 身份探针 4 组符合预期（真 admin / 真 operator 放行；无成员、未登录被拒）';
END
$probe$;
...
DO $fin$
BEGIN
  IF current_setting('opw.mode') = 'rehearsal' THEN
    RAISE NOTICE 'OPW: 排练通过，即将 ROLLBACK —— 生产未被修改';
  ELSE
    RAISE NOTICE 'OPW: 断言全部通过，即将 COMMIT';
  END IF;
END
$fin$;

-- psql 的 \if 只认简单布尔值，故先把判定落成变量再分支。
SELECT (:'mode' = 'rehearsal') AS opw_is_rehearsal \gset
\if :opw_is_rehearsal
ROLLBACK;
\else
COMMIT;
