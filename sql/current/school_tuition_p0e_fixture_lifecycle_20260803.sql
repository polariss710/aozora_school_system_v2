-- Committed School-only synthetic fixture for P0-E multi-session tests.
-- Required p0e_fixture_action: preflight | insert | cleanup | residue.
\set ON_ERROR_STOP on
\pset pager off
\if :{?p0e_fixture_action}
\else
  \echo 'P0E_FIXTURE_ACTION_REQUIRED'
  \quit
\endif
...
select set_config('tuition.p0e_fixture_action',:'p0e_fixture_action',true);

do $fixture$
declare
  v_action constant text:=current_setting('tuition.p0e_fixture_action');
  ...
  v_lessons constant uuid[]:=array[
    ...
    'd0d00000-0000-4000-8000-000000001102'::uuid
  ];
  v_relations constant uuid[]:=array[
    'd0d00000-0000-4000-8000-000000005001'::uuid,
    'd0d00000-0000-4000-8000-000000005002'::uuid
  ];
  v_bill constant uuid:='d0d00000-0000-4000-8000-000000006001';
  v_income constant uuid:='d0d00000-0000-4000-8000-000000007101';
  v_legacy constant uuid:='d0d00000-0000-4000-8000-000000002001';
...
begin
  ...
  if v_action in ('preflight','insert') and exists(
    ...
    union all select 1 from public.school_students where id=v_student or student_code='codex-test-p0e-student'
  ...
  if v_action='insert' then
    ...
    values(v_subject,'codex-test P0-E subject','codex-test',true,v_marker,'班课');
    ...
    values(v_teacher,'codex-test-p0e-teacher','codex-test P0-E teacher','codex-test P0-E teacher',
    ...
    values(v_student,'codex-test-p0e-student','codex-test P0-E student','codex-test P0-E student',
      v_entity,'active','school',0.042,0,v_marker);
    insert into public.school_lesson_records(
      ...
      162500,1,2,'online',v_marker,'2020-08',date_trunc('week',lesson_date::timestamp)::date,
    ...
    ) values(v_settlement,v_student,'2020-07',v_entity,0.042,
      0,0,0,0,0,0,0,0,107.50,0,107.50,
      'locked',v_now,v_marker,0,0,0,0,'student_duration_overage_v1','monthly_settlement_lock');

    select * into strict v_snapshot
    from public.school_build_student_tuition_generation_snapshot(v_student,'2020-08',0.042);
    if v_snapshot.candidate_count<>2 or v_snapshot.previous_settlement_id<>v_settlement then
      raise exception 'P0E_FIXTURE_SNAPSHOT_INVALID';
    end if;
    ...
    insert into public.school_income_records(
      ...
      'codex-test P0-E 学费应收','JPY',v_snapshot.total_fee_jpy,v_snapshot.total_fee_jpy,
      null,null,'JPY',null,'pending',false,null,'Cash待提交',true,v_marker,
      'student_tuition_bill',v_bill,'codex-test P0-E 学费应收',jsonb_build_object(
        'generation_source','student_tuition_atomic_generate_v1',
        'generation_manifest_sha256',v_snapshot.generation_manifest_sha256,
    ...
    if (select count(*) from public.school_students where id=v_student and note=v_marker)<>1
      ...
      raise exception 'P0E_FIXTURE_CLEANUP_OWNERSHIP_FAILED';
    end if;
    perform set_config('tuition.p0e_fixture_cleanup',v_marker,true);
    -- Existing P0-D immutable guards whitelist these fixed UUIDs; P0-E keeps the
    -- same UUID namespace but uses its own exact business marker on every row.
    perform set_config('tuition.p0d_fixture_cleanup',
      'codex-test tuition-p0d-e2e-readiness-20260803',true);
    if to_regclass('public.school_student_tuition_generation_revision_adjustments') is not null then
      execute format(
        'delete from public.school_student_tuition_generation_revision_adjustments where generation_identity_id=$1'
      ) using v_generation;
    end if;
    insert into public.school_tuition_atomic_writer_context(backend_pid,transaction_id,writer_source)
    values(pg_backend_pid(),txid_current(),'student_tuition_atomic_generate_v1');
  ...
  if v_action in ('preflight','cleanup','residue') and exists(
    ...
    union all select 1 from public.school_student_tuition_generation_revision_adjustments where generation_identity_id=v_generation
    union all select 1 from public.school_personal_cash_income_linkage_events where income_record_id=v_income
  ) then raise exception 'P0E_FIXTURE_RESIDUE_NOT_ZERO'; end if;
end;
$fixture$;

select :'p0e_fixture_action' action,
  (select count(*) from public.school_students where id='d0d00000-0000-4000-8000-00000000a001') student_count,
  (select count(*) from public.school_student_tuition_generation_revisions where id='d0d00000-0000-4000-8000-000000004001') revision_count;
commit;
