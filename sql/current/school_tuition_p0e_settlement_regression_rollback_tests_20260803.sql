...
declare
  ...
  v_reissue record;
begin
  -- While an active next-period revision exists, mutation reports Rule A.
  begin
    perform public.school_assert_tuition_settlement_mutable(v_original_settlement);
    raise exception 'EXPECTED_RULE_A_REJECTION_MISSING';
  exception when others then
    if position('TUITION_ACTIVE_PREVIOUS_PERIOD_CLAIM_IMMUTABLE' in sqlerrm)=0 then raise; end if;
  end;
  ...
  insert into public.school_student_monthly_settlements(
    ...
    'codex-test P0-E zero carry claim',created_at,updated_at,unlocked_at,unlock_reason,
  ...
  insert into public.school_lesson_records(
    ...
    v_entity,'15:00','17:00',2,'codex-test P0-E zero carry lifecycle','planned',true,
    'codex-test P0-E zero carry lifecycle','school',1000,2000,1,'online',
    'codex-test P0-E zero carry lifecycle',year_month,
  ...
  select v_bill,student_id,business_entity_id,'2020-10','2020-09',null,0,
    ...
    'codex-test P0-E zero carry claim',app_type,created_by,updated_by,created_at,updated_at,
  ...
  ) values(v_legacy,v_student,'2020-10',v_bill,'codex-test-p0e-zero-claim',
    'atomic_charge','codex-test P0-E zero carry claim','{}'::jsonb);
  ...
  ) values(v_generation,v_student,v_entity,date '2020-10-01',v_legacy,now(),'codex-test P0-E zero carry claim');
  insert into public.school_student_tuition_generation_revisions(
    ...
    'codex-test P0-E zero carry claim',now());
  ...
  set lifecycle_status='voided',voided_at=now(),voided_by_authority='codex-test P0-E zero carry claim'
  where id=v_revision;
  update public.school_student_tuition_bills
  set status='cancelled',cancelled_at=now(),cancelled_reason='codex-test P0-E zero carry claim'
  where id=v_bill;
  delete from public.school_tuition_atomic_writer_context
  where backend_pid=pg_backend_pid() and transaction_id=txid_current();
  ...
  from public.school_relock_student_monthly_settlement(
    v_zero_settlement,'codex-test P0-E zero carry relock after Void'
  );
  ...
  from public.school_reissue_atomic_student_tuition_generation_local(
    ...
    'codex-test P0-E zero carry reissue after relock'
  );
  ...
  -- The synthetic source revision remains active in the outer committed fixture;
  -- P0-E's dedicated rollback test separately proves Void changes its mutation code to Rule B.
end
$tests$;

rollback;
\echo 'P0E_FINAL_RULE_A_B_MATRIX_ROLLED_BACK_8_OF_8'
