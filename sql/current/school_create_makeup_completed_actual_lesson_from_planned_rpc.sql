-- school_create_makeup_completed_actual_lesson_from_planned_rpc.sql
...
-- - actual_minutes is derived from duration_hours because the makeup lesson
--   was completed even when it is not billed.
-- - Student settlement month is inherited from planned.year_month.
-- - Teacher settlement month is derived from makeup actual lesson date as YYYY-MM.
--
-- Not supported:
-- - Free actual creation without planned_lesson_id.
-- - completed or cancelled actual creation.
...
-- - Rollback tests inserted billable and non-billable codex-test
--   makeup_completed actual rows from codex-test planned lessons and left no
--   residue. Non-billable rollback confirmed lesson_fee = 0 and
--   actual_minutes = 120.
...
--   planned lesson b729ae81-3bed-4070-8c49-27efe8d6f797 and makeup_completed
--   actual lesson b057546d-ee07-4b63-ada8-c33544bd9c44.
-- - Source planned lesson b729ae81-3bed-4070-8c49-27efe8d6f797 kept status
--   pending_makeup. Makeup actual lesson b057546d-ee07-4b63-ada8-c33544bd9c44
--   has is_billable = true, lesson_fee = 4400, and actual_minutes = 120.
...
begin
  ...
  if v_planned.status not in ('planned', 'pending_makeup') then
    raise exception '当前 planned 状态不能生成 makeup_completed actual：%。', coalesce(v_planned.status, '');
  end if;

  if v_planned.student_id is null then
    raise exception '预定课时缺少学生，不能生成补课完成 actual。';
  end if;

  if v_planned.teacher_id is null then
    raise exception '预定课时缺少老师，不能生成补课完成 actual。';
  end if;

  if v_planned.subject_id is null then
    raise exception '预定课时缺少科目，不能生成补课完成 actual。';
  end if;

  if v_planned.business_entity_id is null then
    raise exception '预定课时缺少业务归属，不能生成补课完成 actual。';
  end if;
  ...
  v_is_billable := coalesce(p_is_billable, true);
  ...
  if exists (
    ...
    raise exception '目标学生月度结算已锁定，不能生成补课完成 actual。';
  end if;
  ...
  if exists (
    ...
    raise exception '目标老师工资月份已锁定，不能生成补课完成 actual。';
  end if;
...
comment on function public.school_create_makeup_completed_actual_lesson_from_planned(
  uuid,
  date,
  text,
  text,
