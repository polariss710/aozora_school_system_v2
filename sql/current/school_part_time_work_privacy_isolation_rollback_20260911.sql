...
BEGIN
  ...
  LOOP
    ...
    ELSE
      ...
      IF v_acl <> t.acl THEN
        RAISE WARNING 'PTWP_DEP_ACL: % 的 ACL 为 %，期望 %', t.proname, v_acl, t.acl;
      RETURN;
      END IF;
    END IF;
  END LOOP;
...
BEGIN
  ...
  --    形状断言就静默通过了。
  IF NOT FOUND THEN
    RAISE EXCEPTION 'PTWP_RB_INC_POLICY_MISSING: 找不到策略 school_restrict_part_time_work_income';
  END IF;
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'PTWP_RB_EV_POLICY_MISSING: 找不到策略 school_select_income_linkage_events_by_visible_parent';
  END IF;
