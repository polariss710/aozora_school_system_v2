...
BEGIN
  ...
  LOOP
    ...
    IF v_rls IS NOT TRUE OR v_force IS NOT FALSE THEN
      RAISE EXCEPTION 'OPOL_PRE_RLS: % rowsecurity=% force=%（期望 t / f）', t.tbl, v_rls, v_force;
    END IF;
    IF v_acl <> '{postgres=arwdDxtm/postgres,service_role=arwdDxtm/postgres}' THEN
      RAISE EXCEPTION 'OPOL_PRE_ACL: % 的 ACL 为 %，期望 %', t.tbl, v_acl, '{postgres=arwdDxtm/postgres,service_role=arwdDxtm/postgres}';
    END IF;
    ...
    IF v_rows <> 0 THEN
      RAISE EXCEPTION 'OPOL_PRE_ROWS: % 有 % 行（取证时为 0）—— 前提已变，停止', t.tbl, v_rows;
    END IF;

    SELECT count(*) INTO v_n FROM pg_policies
     WHERE schemaname='public' AND tablename=t.tbl;
    IF v_n <> 1 THEN
      RAISE EXCEPTION 'OPOL_PRE_POLICY_COUNT: % 上有 % 条策略（期望 1）', t.tbl, v_n;
    END IF;
    ...
    IF NOT FOUND THEN
      RAISE EXCEPTION 'OPOL_PRE_POLICY_MISSING: % 上找不到策略 %', t.tbl, t.pol;
    END IF;
    IF p.permissive <> 'PERMISSIVE' OR p.cmd <> 'ALL'
      ...
      RAISE EXCEPTION 'OPOL_PRE_POLICY_SHAPE: % 的策略形状不符  permissive=% cmd=% roles=% qual=% check=%',
        t.tbl, p.permissive, p.cmd, p.roles::text, p.qual, p.with_check;
    END IF;
  END LOOP;
  RAISE NOTICE 'OPOL_PRE: 10 张表的 RLS 开关 / ACL / 行数 / 策略集合 全部符合预期';
END
$pre$;

-- ===== 删除 10 条全放行策略（RLS 保持启用 ⇒ 默认拒绝）=====
DROP POLICY school_allow_all_actual_lessons ON public.school_actual_lessons;
DROP POLICY school_allow_all_import_batches ON public.school_import_batches;
DROP POLICY school_allow_all_import_errors ON public.school_import_errors;
DROP POLICY school_allow_all_lesson_schedules ON public.school_lesson_schedules;
DROP POLICY school_allow_all_monthly_reports ON public.school_monthly_reports;
DROP POLICY school_allow_all_planned_lessons ON public.school_planned_lessons;
DROP POLICY school_allow_all_schedule_students ON public.school_schedule_students;
DROP POLICY school_allow_all_student_months ON public.school_student_months;
DROP POLICY school_allow_all_student_payments ON public.school_student_payments;
DROP POLICY school_allow_all_teacher_work_logs ON public.school_teacher_work_logs;
...
BEGIN
  ...
  LOOP
    ...
    IF v_rls IS NOT TRUE OR v_force IS NOT FALSE THEN
      RAISE EXCEPTION 'OPOL_POST_RLS: % rowsecurity=% force=%（期望 t / f）', t.tbl, v_rls, v_force;
    END IF;
    IF v_acl <> '{postgres=arwdDxtm/postgres,service_role=arwdDxtm/postgres}' THEN
      RAISE EXCEPTION 'OPOL_POST_ACL: % 的 ACL 为 %，期望 %', t.tbl, v_acl, '{postgres=arwdDxtm/postgres,service_role=arwdDxtm/postgres}';
    END IF;
    ...
    EXECUTE format('SELECT count(*) FROM public.%I', t.tbl) INTO v_rows;
    IF v_rows <> 0 THEN
      RAISE EXCEPTION 'OPOL_POST_ROWS: % 有 % 行（取证时为 0）—— 前提已变，停止', t.tbl, v_rows;
    END IF;

    SELECT count(*) INTO v_n FROM pg_policies
     WHERE schemaname='public' AND tablename=t.tbl;
    IF v_n <> 0 THEN
      RAISE EXCEPTION 'OPOL_POST_POLICY_COUNT: % 上有 % 条策略（期望 0）', t.tbl, v_n;
    END IF;
  END LOOP;
  RAISE NOTICE 'OPOL_POST: 10 张表的 RLS 开关 / ACL / 行数 / 策略集合 全部符合预期';
END
$post$;
