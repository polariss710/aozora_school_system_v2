-- ###########################################################################
-- ##  已部署 —— 2026-09-04 16:47 JST，验收全部通过，无持久化业务数据改动
-- ##
-- ##  六个函数的实际 proacl / owner / 参数数与本文件第二节的对照表逐行相符；
-- ##  五个新函数正文与本文件逐字一致；**classroom 检查仍在生产 prosrc 中**（E8）。
-- ##
-- ##  两条 P1 的判据均成功：
-- ##    B-1 CNY submitted → submitted / version=2   （首版必然 PAYLOAD_CONFLICT）
-- ##    B-3 CNY approved  → approved_fixed / paid    （首版必然 APPROVAL_EVIDENCE_CONFLICT）
-- ##
-- ##  C 组六条全部返回预期错误码；九条基线断言证伪均实际触发 ABORT；
-- ##  JPY 旧调用与基线返回逐字段一致；幂等与 prepared 恢复照旧。
-- ##  13 个测试支出全部回滚，残留 0，attempt 表仍 25 行。
-- ##
-- ##  PostgREST 缓存已于 16:47:47 用 service_role 实调 prepare / submitted /
-- ##  rejected 验证（携带新增参数，返回 22023 而非 PGRST202）。
-- ##
-- ##  完整验收报告（1004 行）归档于
-- ##  ~/aozora-security-20260827/school-cross-currency-deployment-report-20260904-1649.md
-- ##  SHA-256 32d50c047e5982b65e4f0393aba0f4dc908ea01c9d0aa791d3a2e748e029363c
-- ##
-- ##  **注意：跨币种此时仍无法从界面发起。** Edge 与前端尚未改动，
-- ##  前端的 cashCardUnavailableReason() 仍把 CNY 卡置灰。本文件解决的是
-- ##  「后端能不能表达并回写跨币种」，不是「能不能用」。
-- ###########################################################################
