-- ===========================================================================
-- 学费 lesson_count 语义修复：v1（组内序号求和）→ v2（课时条数）
--   部署脚本 / 2026-09-06
-- ===========================================================================
--
-- 设计文档：docs/school-v2-lesson-count-semantics-fix-design-20260906.md（第二稿）
-- 审查报告：~/aozora-security-20260827/
--            lesson-count-design-review-20260906-12:05-report.md
--
-- 【定义来源】以下六个函数的正文取自**生产** pg_get_functiondef(oid)
--   （取证 2026-09-06 12:05:12 JST），逐字节 md5 已与审查报告 Appendix C 核对一致，
--   再在其上施加本次补丁。**没有使用仓库里的 sql/current/*.sql**——
--   生产可能被热修复过而那次修复从未回落仓库（lessons B3），
--   照仓库重建会把已修好的缺陷静默装回去。
--
-- 【用法】
--   彩排（生产零变更，但真解析、真部署、真跑完断言，结束时 ROLLBACK）：
--     psql "$DB_URL" -v mode=rehearsal -f <本文件>
--   正式部署：
--     psql "$DB_URL" -v mode=commit    -f <本文件>
--   不传 mode 时默认 rehearsal。
--
-- 【六个函数必须在同一事务内部署】分批会产生三种真实错误中间态（设计 §9.1）：
--   · 只放宽候选      → NULL 序号进入明细 NOT NULL 路径，失败
--   · 只改 builder    → count 4 / sum 6 按 v1 校验失败；恰好相等时静默存下漏标账单
--   · 只让 writer 标 v2 → 旧 validator 可能放行语义错误的账单
--   同一事务只消除「已提交目录的中间态」，**不隔离在途调用**。
--   而且本脚本跑在 REPEATABLE READ 下，两次全量扫描读的是**事务开始时的快照**：
--   晚于快照建立的提交，即使发生在扫描语句之前也看不到。
--   所以顺序只能是「**先排空，再开事务**」，不能开了事务再等 writer 结束。
--   排空要覆盖全部真实入口（页面、服务端、定时任务、重试队列、直接运维连接），
--   并持续阻断到结果验证结束；一次 pg_stat_activity 空列表不阻止下一秒的新调用。
--
-- 【本脚本不做】不改任何历史账单与冻结快照、不回填历史课时 lesson_count、
--   不补建历史月份账单、不改 school_p0c_baseline_* 系列及其 ACL、
...
\echo ''
\echo '================================================================'
\echo 'lesson_count 语义修复 v1 -> v2  部署'
\echo 'mode =' :mode
\echo '================================================================'
...
-- ---------------------------------------------------------------------------
CREATE TEMP TABLE lc2_run(mode text NOT NULL) ON COMMIT DROP;
INSERT INTO lc2_run(mode) VALUES (:'mode');
...
-- ---------------------------------------------------------------------------
-- §2 期望标记表（strpos 精确子串；不用 LIKE，避免下划线被当通配符）
--   must_exist=true  → 部署/回滚后必须出现
--   must_exist=false → 部署/回滚后必须消失
-- ---------------------------------------------------------------------------
CREATE TEMP TABLE lc2_marker(
  fn_key text NOT NULL, needle text NOT NULL, must_exist boolean NOT NULL
) ON COMMIT DROP;

INSERT INTO lc2_marker(fn_key,needle,must_exist) VALUES
  ('candidates','evidence.lesson_count IS NOT NULL',false),
  ('candidates','evidence.lesson_count > 0',false),
  ('build','ranked_candidates',true),
  ('build','bill_lesson_ordinal',true),
  ('build','source_start_time',true),
  ('build','count(*)::integer AS lesson_count',true),
  ('build','sum(detail.lesson_count)',false),
  ('validator','lesson_count_semantics',true),
  ('validator','v_expected_lesson_total',true),
  ('validator','TUITION_LESSON_COUNT_SEMANTICS_DIVERGED',true),
  ('validator','DISTINCT FROM v_lesson_count',false),
  ('base_core_v1','lesson_count_semantics',true),
  ('next_revision_core','lesson_count_semantics',true),
  ('next_revision_p0e_core','lesson_count_semantics',true);

-- ---------------------------------------------------------------------------
-- §3 前置断言 A：六个函数必须逐字节等于审查基线
...
-- ---------------------------------------------------------------------------
-- §4 前置断言 B：builder 新增的排序列必须真实存在且为 text
--   第一稿曾凭空使用 start_time；它确实在底表上，但 candidate_rows 从未投影它，
--   且该列可空、生产 515 条 planned 中 373 条 NULL、2 条空串（lessons E10）。
-- ---------------------------------------------------------------------------
DO $do$
DECLARE v_type text;
BEGIN
  ...
  FROM pg_attribute a
  WHERE a.attrelid = 'public.school_lesson_records'::regclass
    AND a.attname = 'start_time' AND a.attnum > 0 AND NOT a.attisdropped;
  IF v_type IS NULL THEN
    RAISE EXCEPTION 'LC2_START_TIME_MISSING: school_lesson_records.start_time 不存在';
  END IF;
  IF v_type <> 'text' THEN
    RAISE EXCEPTION 'LC2_START_TIME_TYPE_UNEXPECTED: 期望 text，实际 %。'
      '排序表达式 nullif(btrim(...),'''')::time 是按 text 可空写的，类型变了必须重审。', v_type;
  END IF;
END
$do$;
...
-- ---------------------------------------------------------------------------
-- §5 盘点项（只报告，不设硬停止）
--   本次要往 bill / income 的 source_snapshot 顶层加一个新键
--   lesson_count_semantics。如果这两张表上有约束限制该 JSON 的键集合，
--   插入会失败。我不知道这里应该是什么，所以它是盘点项不是断言项（lessons E10）。
-- ---------------------------------------------------------------------------
\echo '--- 盘点：bills / income_records 上引用 source_snapshot 的 CHECK 约束 ---'
SELECT c.conrelid::regclass AS "表", c.conname AS "约束", pg_get_constraintdef(c.oid) AS "定义"
FROM pg_constraint c
WHERE c.contype = 'c'
  AND c.conrelid IN ('public.school_student_tuition_bills'::regclass,
                     'public.school_income_records'::regclass)
  AND strpos(pg_get_constraintdef(c.oid), 'source_snapshot') > 0
ORDER BY 1, 2;
...
-- (a) 记录部署后 md5，并断言六个定义全部真的变了
DO $do$
DECLARE r record; v_md5 text; v_unchanged text := '';
BEGIN
  FOR r IN SELECT * FROM lc2_target ORDER BY fn_key LOOP
    v_md5 := md5(pg_get_functiondef(to_regprocedure(r.signature)::oid));
    UPDATE lc2_target SET after_md5 = v_md5 WHERE fn_key = r.fn_key;
    IF v_md5 = r.before_md5 THEN
      v_unchanged := v_unchanged || E'\n  ' || r.signature;
    END IF;
  END LOOP;
  IF v_unchanged <> '' THEN
    RAISE EXCEPTION 'LC2_NO_OP: 以下函数定义未发生变化，说明 CREATE OR REPLACE 没有生效：%', v_unchanged;
  END IF;
END
$do$;
...
BEGIN
  ...
  END LOOP;
  IF v_bad <> '' THEN
    RAISE EXCEPTION 'LC2_MARKER_MISMATCH: 部署结果与预期不符。%', v_bad;
  END IF;
END
$do$;

-- (c) ACL / owner / 安全属性必须与部署前完全一致
DO $do$
DECLARE v_bad text := '';
BEGIN
...
-- (e) 输出新的 md5，作为后续漂移检查与回滚脚本的基线
\echo '--- 六函数 md5：部署前 → 部署后 ---'
SELECT fn_key, before_md5 AS "前", after_md5 AS "后", purpose AS "改动"
FROM lc2_target ORDER BY fn_key;

-- ---------------------------------------------------------------------------
-- §11 收尾
--   rehearsal → ROLLBACK（生产零变更，但已经真解析、真部署、真跑完全部断言）
--   commit    → COMMIT
-- ---------------------------------------------------------------------------
...
COMMIT;
\echo ''
\echo '>>> COMMIT 已执行：六个函数已在同一事务内切换到 v2（条数语义）。'
\else
ROLLBACK;
\echo ''
