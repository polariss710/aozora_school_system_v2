-- Committed School-only synthetic fixture for P0-D multi-session tests.
-- Required p0d_fixture_action: preflight | insert | cleanup | residue.
\set ON_ERROR_STOP on
\pset pager off
\if :{?p0d_fixture_action}
\else
  \echo 'P0D_FIXTURE_ACTION_REQUIRED'
  \quit
\endif
...
select set_config('tuition.p0d_fixture_action',:'p0d_fixture_action',true);

do $fixture$
declare
  v_action constant text:=current_setting('tuition.p0d_fixture_action');
  v_marker constant text:='codex-test tuition-p0d-e2e-readiness-20260803';
...
begin
  if v_action not in ('preflight','insert','cleanup','residue') then
    raise exception 'P0D_FIXTURE_ACTION_INVALID';
  end if;
  if v_action in ('preflight','insert') and exists(
    select 1 from public.school_business_entities where id=v_entity or code='codex-test-tuition-p0d-e2e-readiness'
    union all select 1 from public.school_subjects where id=v_subject or name='codex-test P0-D subject'
    union all select 1 from public.school_teachers where id=v_teacher or teacher_code='codex-test-p0d-teacher'
    union all select 1 from public.school_students where id=v_student or student_code='codex-test-p0d-student'
  ...
  ) then raise exception 'P0D_FIXTURE_COLLISION'; end if;

  if v_action='insert' then
    insert into public.school_business_entities(id,code,name,entity_type,default_currency,is_active,note)
    values(v_entity,'codex-test-tuition-p0d-e2e-readiness','codex-test P0-D entity','company','JPY',true,v_marker);
    insert into public.school_subjects(id,name,category,is_active,note,primary_category)
    values(v_subject,'codex-test P0-D subject','codex-test',true,v_marker,'班课');
    ...
    values(v_teacher,'codex-test-p0d-teacher','codex-test P0-D teacher','codex-test P0-D teacher',
    ...
    values(v_student,'codex-test-p0d-student','codex-test P0-D student','codex-test P0-D student',
    ...
    if v_snapshot.candidate_count<>2 or v_snapshot.previous_settlement_id<>v_settlement then
      raise exception 'P0D_FIXTURE_SNAPSHOT_INVALID';
    end if;
    ...
    insert into public.school_income_records(
      ...
      'codex-test P0-D 学费应收','JPY',v_snapshot.total_fee_jpy,v_snapshot.total_fee_jpy,
      null,null,'JPY',null,'pending',false,null,'Cash待提交',true,v_marker,
      'student_tuition_bill',v_bill,'codex-test P0-D 学费应收',jsonb_build_object(
        'generation_source','student_tuition_atomic_generate_v1',
        'generation_manifest_sha256',v_snapshot.generation_manifest_sha256,
    ...
    if (select count(*) from public.school_students where id=v_student and note=v_marker)<>1
      ...
      raise exception 'P0D_FIXTURE_CLEANUP_OWNERSHIP_FAILED';
    end if;
    perform set_config('tuition.p0d_fixture_cleanup',v_marker,true);
    insert into public.school_tuition_atomic_writer_context(backend_pid,transaction_id,writer_source)
    values(pg_backend_pid(),txid_current(),'student_tuition_atomic_generate_v1');
...
select :'p0d_fixture_action' action,
