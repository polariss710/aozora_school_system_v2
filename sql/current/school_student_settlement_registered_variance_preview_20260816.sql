-- School V2 monthly settlement registered lesson variance Preview extension.
-- Execute atomically with:
--   psql "$SCHOOL_SUPABASE_DB_URL" -X -v ON_ERROR_STOP=1 -1 -f <this-file>
-- This migration changes one read-only wrapper definition and performs no business DML.
...
begin
  select md5(pg_get_functiondef(v_signature::oid)) into v_definition_md5;
  if v_definition_md5 not in (
    '44c998671550d2288c7f4960d6d52fdc',
    '13fe9c288069ae785887559e6b475138'
  ) then
    raise exception 'REGISTERED_VARIANCE_PREVIEW_DEFINITION_DRIFT: %', v_definition_md5;
  end if;
  if pg_get_userbyid((select p.proowner from pg_proc p where p.oid=v_signature::oid)) <> 'postgres'
    ...
    raise exception 'REGISTERED_VARIANCE_PREVIEW_SECURITY_CONTRACT_DRIFT';
  end if;
end
$preflight$;
...
declare
  ...
  v_registered_pending_hours numeric := 0;
  v_registered_pending_amount_jpy numeric := 0;
  v_registered_overage_hours numeric := 0;
  v_registered_overage_amount_jpy numeric := 0;
  v_registered_overage_amount_cny numeric;
  v_registered_source_count integer := 0;
  v_registered_overage_source_count integer := 0;
  v_registered_source_manifest_material text := '';
  v_registered_net_direction text;
  v_registered_net_hours numeric := 0;
  v_registered_net_amount_jpy numeric := 0;
  v_unresolved_planned_count integer := 0;
  v_unresolved_manifest_material text := '';
  v_variance_summary_status text;
  v_variance_summary_manifest text;
begin
  ...
  select
    coalesce(-sum(source_row.source_hours)
      filter (where source_row.source_type='unused_planned_credit_v1'), 0),
    coalesce(-sum(source_row.source_amount_jpy)
      filter (where source_row.source_type='unused_planned_credit_v1'), 0),
    coalesce(sum(source_row.source_hours)
      filter (where source_row.source_type='actual_duration_overage_charge_v1'), 0),
    coalesce(sum(source_row.source_amount_jpy)
      filter (where source_row.source_type='actual_duration_overage_charge_v1'), 0),
    sum(source_row.source_amount_cny)
      filter (where source_row.source_type='actual_duration_overage_charge_v1'),
    count(*)::integer,
    count(*) filter (
      where source_row.source_type='actual_duration_overage_charge_v1'
    )::integer,
    coalesce(string_agg(
      source_row.line_manifest_sha256,
      '' order by source_row.source_type,
        source_row.source_planned_lesson_id,
        source_row.source_actual_lesson_id
    ), '')
  into
    v_registered_pending_hours,
    v_registered_pending_amount_jpy,
    v_registered_overage_hours,
    v_registered_overage_amount_jpy,
    v_registered_overage_amount_cny,
    v_registered_source_count,
    v_registered_overage_source_count,
    v_registered_source_manifest_material
  from public.school_tuition_p0f_source_lines(
    p_student_id,
    p_business_entity_id,
    p_year_month,
    v_legacy.exchange_rate,
    false
  ) source_row;

  if v_registered_overage_source_count = 0 then
    v_registered_overage_amount_cny := 0;
  end if;

  select
    count(*)::integer,
    coalesce(string_agg(planned_row.id::text, '' order by planned_row.id), '')
  into v_unresolved_planned_count, v_unresolved_manifest_material
  from public.school_lesson_records planned_row
  where planned_row.app_type='school'
    and planned_row.lesson_type='planned'
    and planned_row.student_id=p_student_id
    and planned_row.business_entity_id=p_business_entity_id
    and public.school_resolve_r1d_e_c_lesson_student_month(planned_row.id)=p_year_month
    and planned_row.voided_at is null
    and coalesce(planned_row.is_billable,true) is true
    and planned_row.status not in (
    ...
    and not exists (
      ...
      where actual_row.app_type='school'
        ...
        and actual_row.planned_lesson_id=planned_row.id
        and (
          actual_row.status in ('completed','makeup_completed','cancelled')
          or actual_row.is_billable is false
        )
    );

  v_registered_net_direction := case
    when round(v_registered_overage_amount_jpy-v_registered_pending_amount_jpy,2) < 0
      then 'pending'
    when round(v_registered_overage_amount_jpy-v_registered_pending_amount_jpy,2) > 0
      then 'overage'
    when round(v_registered_overage_hours-v_registered_pending_hours,6) < 0
      then 'pending'
    when round(v_registered_overage_hours-v_registered_pending_hours,6) > 0
      then 'overage'
    else 'balanced'
  end;
  v_registered_net_hours := round(
    abs(v_registered_overage_hours-v_registered_pending_hours), 6
  );
  v_registered_net_amount_jpy := round(
    abs(v_registered_overage_amount_jpy-v_registered_pending_amount_jpy), 2
  );
  v_variance_summary_status := case
    when v_registered_source_count=0 then 'empty'
    when v_registered_overage_source_count>0
      and (v_legacy.exchange_rate is null or v_registered_overage_amount_cny is null)
      then 'unavailable'
    else 'ready'
  end;
  v_variance_summary_manifest := encode(extensions.digest(concat_ws('|',
    'registered_lesson_variance_summary_v1',
    p_student_id::text,
    p_business_entity_id::text,
    p_year_month,
    v_variance_summary_status,
    to_char(v_registered_pending_hours,'FM999999990.000000'),
    to_char(v_registered_pending_amount_jpy,'FM999999990.00'),
    to_char(v_registered_overage_hours,'FM999999990.000000'),
    to_char(v_registered_overage_amount_jpy,'FM999999990.00'),
    coalesce(to_char(v_registered_overage_amount_cny,'FM999999990.00'),''),
    v_registered_net_direction,
    to_char(v_registered_net_hours,'FM999999990.000000'),
    to_char(v_registered_net_amount_jpy,'FM999999990.00'),
    v_registered_source_count::text,
    v_unresolved_planned_count::text,
    v_registered_source_manifest_material,
    v_unresolved_manifest_material
  ),'sha256'),'hex');
  ...
  return jsonb_build_object(
    ...
    'preview',to_jsonb(v_preview) || jsonb_build_object(
      ...
      'registered_pending_hours',round(v_registered_pending_hours,6),
      'registered_pending_amount_jpy',round(v_registered_pending_amount_jpy,2),
      'registered_overage_hours',round(v_registered_overage_hours,6),
      'registered_overage_amount_jpy',round(v_registered_overage_amount_jpy,2),
      'registered_overage_amount_cny',round(v_registered_overage_amount_cny,2),
      'registered_net_direction',v_registered_net_direction,
      'registered_net_hours',v_registered_net_hours,
      'registered_net_amount_jpy',v_registered_net_amount_jpy,
      'registered_source_count',v_registered_source_count,
      'unresolved_planned_count',v_unresolved_planned_count,
      'registered_overage_included_in_system_difference',
        v_preview.source_treatment_mode='separate_makeup_and_overage_v1'
        and v_variance_summary_status='ready'
        and v_registered_overage_source_count>0,
      'variance_summary_status',v_variance_summary_status,
      'variance_summary_manifest_sha256',v_variance_summary_manifest
    ),
...
comment on function public.school_preview_student_settlement_adjustment_dialog(
  uuid,uuid,text,text,numeric,text,date,text,numeric
) is 'Pure read-only P0-F dialog preview. Preserves financial mode fields and adds a DB-authoritative registered lesson variance summary for separate mode UI expression; performs no business DML.';
...
begin
  if pg_get_userbyid((select p.proowner from pg_proc p where p.oid=v_signature::oid)) <> 'postgres'
    ...
    raise exception 'REGISTERED_VARIANCE_PREVIEW_POSTFLIGHT_CONTRACT_FAILED';
  end if;
end
$postflight$;
