-- Committed write test against the fixed, explicitly marked P0-E whitelist fixture.
...
begin
  if (select count(*) from public.school_students where id=v_student and note=v_marker)<>1
    ...
    raise exception 'LOCKED_CARRY_WHITELIST_OWNERSHIP_INVALID';
  end if;
  ...
  from public.school_void_atomic_student_tuition_generation_local(
    v_revision,v_bill,v_income,v_manifest,v_marker||' locked carry commit Void');
  ...
  from public.school_reissue_atomic_student_tuition_generation_local(
    ...
    v_marker||' locked carry commit Reissue');
  select * into strict v_duplicate
  from public.school_reissue_atomic_student_tuition_generation_local(
    v_generation,v_revision,v_student,v_entity,'2020-08',
    v_snapshot.candidate_manifest_sha256,v_snapshot.generation_manifest_sha256,
    0.043,v_snapshot.total_fee_jpy,v_snapshot.billing_amount_cny,
    v_marker||' locked carry commit duplicate');
  if v_reissue.idempotent or not v_duplicate.idempotent
     or v_reissue.previous_carryover_cny<>107.50
     or v_reissue.billing_amount_cny<>28057.50
     or v_duplicate.tuition_bill_id<>v_reissue.tuition_bill_id
    ...
    raise exception 'LOCKED_CARRY_WHITELIST_RESULT_INVALID';
  end if;
...
commit;
select 'LOCKED_CARRY_ORDINARY_REISSUE_COMMIT_PASS' result;
