...
BEGIN
  ...
  LOOP
    ...
    END IF;
  END LOOP;
  RAISE NOTICE 'PTWP_DEP: membership 与 PTW admin 守卫在位且与取证一致';
END
$dep$;
...
BEGIN
  ...
  LOOP
    SELECT count(*) INTO v_n FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
     WHERE n.nspname='public' AND p.proname=t.proname;
    IF v_n <> 1 THEN
      RAISE EXCEPTION 'PTWP_PRE_ARITY: public.% 有 % 个（应为 1）', t.proname, v_n;
    END IF;
    SELECT md5(pg_get_functiondef(p.oid)), coalesce(p.proacl::text,'') INTO v_md5, v_acl
      FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
     WHERE n.nspname='public' AND p.proname=t.proname;
    IF v_md5 <> t.md5 THEN
      RAISE EXCEPTION 'PTWP_PRE_MD5: % 的定义为 %，期望 %', t.proname, v_md5, t.md5;
    END IF;
    IF v_acl <> t.acl THEN
      RAISE EXCEPTION 'PTWP_PRE_ACL: % 的 ACL 为 %，期望 %', t.proname, v_acl, t.acl;
    END IF;
  END LOOP;
  ...
  IF v_md5 <> '48a6c54693e746c7426caec01e8056c1' THEN
    RAISE EXCEPTION 'PTWP_PRE_E_MD5: school_update_personal_cash_income_linkage_event_status 的定义为 %，期望 %（本脚本不改它的函数体）', v_md5, '48a6c54693e746c7426caec01e8056c1';
  END IF;
  IF v_acl <> '{postgres=X/postgres,authenticated=X/postgres,service_role=X/postgres}' THEN
    RAISE EXCEPTION 'PTWP_PRE_E_ACL: school_update_personal_cash_income_linkage_event_status 的 ACL 为 %，期望 %', v_acl, '{postgres=X/postgres,authenticated=X/postgres,service_role=X/postgres}';
  END IF;

  RAISE NOTICE 'PTWP_PRE: 9 个函数的 md5/ACL 与 school_update_personal_cash_income_linkage_event_status 的 md5/ACL 全部符合预期';
END
$pre$;
...
BEGIN
  ...
  LOOP
    ...
    IF v_a <> t.n_admin OR v_o <> t.n_operator THEN
      RAISE EXCEPTION 'PTWP_PRE_GUARD_SHAPE: % 的 PTW admin 守卫 % 次（期望 %）、operator 守卫 % 次（期望 %）',
        t.proname, v_a, t.n_admin, v_o, t.n_operator;
    END IF;
  END LOOP;
  RAISE NOTICE 'PTWP_PRE: 8 个打工函数的守卫调用形状符合预期';
END
$preg$;
...
BEGIN
  ...
  IF v_rls IS NOT TRUE OR v_force IS NOT FALSE THEN
    RAISE EXCEPTION 'PTWP_PRE_INC_RLS: school_income_records rowsecurity=% force=%（期望 t / f）', v_rls, v_force;
  END IF;
  ...
  IF NOT FOUND OR p.permissive <> 'PERMISSIVE' OR p.qual <> '((status <> ''incident_quarantined''::text) AND (operational_excluded IS NOT TRUE))' THEN
    RAISE EXCEPTION 'PTWP_PRE_INC_OLD_POLICY: 原有 school_select_operational_income_records 缺失或已漂移（本脚本不碰它）';
  END IF;
  SELECT count(*) INTO v_n FROM pg_policies WHERE schemaname='public' AND tablename='school_income_records';
  IF v_n <> 1 THEN
    RAISE EXCEPTION 'PTWP_PRE_INC_POLICY_COUNT: school_income_records 上有 % 条策略（期望 1）', v_n;
  END IF;
  ...
  IF v_rls IS NOT FALSE OR v_force IS NOT FALSE THEN
    RAISE EXCEPTION 'PTWP_PRE_EV_RLS: school_personal_cash_income_linkage_events rowsecurity=% force=%（期望 f / f）', v_rls, v_force;
  END IF;
  IF v_acl <> '{postgres=arwdDxtm/postgres,authenticated=rm/postgres,service_role=arwdDxtm/postgres}' THEN
    RAISE EXCEPTION 'PTWP_PRE_EV_ACL: school_personal_cash_income_linkage_events 的授权为 %，期望 %（本脚本不改它的授权）', v_acl, '{postgres=arwdDxtm/postgres,authenticated=rm/postgres,service_role=arwdDxtm/postgres}';
  END IF;
  SELECT count(*) INTO v_n FROM pg_policies WHERE schemaname='public' AND tablename='school_personal_cash_income_linkage_events';
  IF v_n <> 0 THEN
    RAISE EXCEPTION 'PTWP_PRE_EV_POLICY_COUNT: school_personal_cash_income_linkage_events 上有 % 条策略（期望 0）', v_n;
  END IF;

  RAISE NOTICE 'PTWP_PRE: 两张表的 RLS 开关、授权与策略集合符合预期';
END
$prep$;

-- ===== A. 收入表：RESTRICTIVE 策略 =====
CREATE POLICY school_restrict_part_time_work_income
  ON public.school_income_records
  AS RESTRICTIVE FOR SELECT TO public
  USING (
    (source_type IS DISTINCT FROM 'part_time_work'
     AND income_category IS DISTINCT FROM 'part_time_work')
    OR EXISTS (
      SELECT 1 FROM public.school_get_current_app_membership() membership(user_id, role, is_active)
       WHERE membership.is_active AND membership.role = 'admin'
    )
  );
COMMENT ON POLICY school_restrict_part_time_work_income ON public.school_income_records IS 'Hides the owner external part-time income from every role except an active admin. RLS only covers invoker paths; postgres and service_role have BYPASSRLS, so SECURITY DEFINER readers are deliberately unaffected.';

-- ===== C. Cash 收入关联表：启用 RLS 并加策略 =====
--
-- ⚠️ 判据写成【正向】：「父收入行可读 ⇒ 本事件可读」。
--    不能写成 NOT EXISTS（打工父记录）——父表的新策略会把打工行藏起来，
--    子查询随之为空，否定式反而为真，事件会被【放行】，方向正好相反。
--    （Codex 2026-09-11 判定：危险的是否定式，不是子查本身。）
--
-- admin 单独放行，使孤儿事件与事故隔离父记录的事件对他仍然可见。
ALTER TABLE public.school_personal_cash_income_linkage_events ENABLE ROW LEVEL SECURITY;
CREATE POLICY school_select_income_linkage_events_by_visible_parent
  ON public.school_personal_cash_income_linkage_events
  AS PERMISSIVE FOR SELECT TO public
  USING (
    EXISTS (SELECT 1 FROM public.school_income_records i WHERE i.id = income_record_id)
    OR EXISTS (
      SELECT 1 FROM public.school_get_current_app_membership() membership(user_id, role, is_active)
       WHERE membership.is_active AND membership.role = 'admin'
    )
  );
COMMENT ON POLICY school_select_income_linkage_events_by_visible_parent ON public.school_personal_cash_income_linkage_events IS 'Shows a Cash income linkage event only when its parent income row is itself readable, so hiding part-time income hides its events too. Written as a positive condition on purpose: a NOT EXISTS form would be inverted by the parent policy and expose the rows it meant to hide. Active admins are exempt so that orphan and quarantined-parent events stay visible to them.';

-- ===== E. 撤销 authenticated 的 EXECUTE =====
--
-- 它的兄弟函数（school_mark_cash_income_confirmed / _rejected / _request_submitted）
-- ACL 全是 {postgres,service_role}，唯独它多一个 authenticated，像是遗漏。
-- 前端与线上 9 个 Edge 均零调用，撤掉不影响任何已知路径；函数体不改。
REVOKE EXECUTE ON FUNCTION public.school_update_personal_cash_income_linkage_event_status(uuid,text,uuid,text) FROM authenticated;
...
begin
  ...
  if not exists (
    ...
    raise exception using errcode='42501',message='BE_UI_ACTIVE_ADMIN_REQUIRED';
  end if;
...
BEGIN
  ...
  LOOP
    SELECT count(*) INTO v_n FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
     WHERE n.nspname='public' AND p.proname=t.proname;
    IF v_n <> 1 THEN
      RAISE EXCEPTION 'PTWP_POST_ARITY: public.% 有 % 个（应为 1）', t.proname, v_n;
    END IF;
    SELECT md5(pg_get_functiondef(p.oid)), coalesce(p.proacl::text,'') INTO v_md5, v_acl
      FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
     WHERE n.nspname='public' AND p.proname=t.proname;
    IF v_md5 <> t.md5 THEN
      RAISE EXCEPTION 'PTWP_POST_MD5: % 的定义为 %，期望 %', t.proname, v_md5, t.md5;
    END IF;
    IF v_acl <> t.acl THEN
      RAISE EXCEPTION 'PTWP_POST_ACL: % 的 ACL 为 %，期望 %', t.proname, v_acl, t.acl;
    END IF;
  END LOOP;
  ...
  SELECT md5(pg_get_functiondef(p.oid)), coalesce(p.proacl::text,'') INTO v_md5, v_acl
   ...
   WHERE n.nspname='public' AND p.proname='school_update_personal_cash_income_linkage_event_status';
  IF v_md5 <> '48a6c54693e746c7426caec01e8056c1' THEN
    RAISE EXCEPTION 'PTWP_POST_E_MD5: school_update_personal_cash_income_linkage_event_status 的定义为 %，期望 %（本脚本不改它的函数体）', v_md5, '48a6c54693e746c7426caec01e8056c1';
  END IF;
  IF v_acl <> '{postgres=X/postgres,service_role=X/postgres}' THEN
    RAISE EXCEPTION 'PTWP_POST_E_ACL: school_update_personal_cash_income_linkage_event_status 的 ACL 为 %，期望 %', v_acl, '{postgres=X/postgres,service_role=X/postgres}';
  END IF;

  RAISE NOTICE 'PTWP_POST: 9 个函数的 md5/ACL 与 school_update_personal_cash_income_linkage_event_status 的 md5/ACL 全部符合预期';
END
$post$;
...
BEGIN
  ...
  LOOP
    ...
    -- ⚠️ 'school_require_current_part_time_work_admin' 不是 'school_require_current_part_time_work_operator' 的子串，两个计数互不干扰
    IF v_a <> t.n_admin OR v_o <> t.n_operator THEN
      RAISE EXCEPTION 'PTWP_POST_GUARD_SHAPE: % 的 PTW admin 守卫 % 次（期望 %）、operator 守卫 % 次（期望 %）',
        t.proname, v_a, t.n_admin, v_o, t.n_operator;
    END IF;
  END LOOP;
  RAISE NOTICE 'PTWP_POST: 8 个打工函数的守卫调用形状符合预期';
END
$postg$;
...
BEGIN
  ...
  SELECT c.relrowsecurity, c.relforcerowsecurity INTO v_rls, v_force
    FROM pg_class c WHERE c.oid='public.school_income_records'::regclass;
  IF v_rls IS NOT TRUE OR v_force IS NOT FALSE THEN
    RAISE EXCEPTION 'PTWP_POST_INC_RLS: school_income_records rowsecurity=% force=%（期望 t / f）', v_rls, v_force;
  END IF;
  SELECT * INTO p FROM pg_policies
   WHERE schemaname='public' AND tablename='school_income_records' AND policyname='school_select_operational_income_records';
  IF NOT FOUND OR p.permissive <> 'PERMISSIVE' OR p.qual <> '((status <> ''incident_quarantined''::text) AND (operational_excluded IS NOT TRUE))' THEN
    RAISE EXCEPTION 'PTWP_POST_INC_OLD_POLICY: 原有 school_select_operational_income_records 缺失或已漂移（本脚本不碰它）';
  END IF;
  SELECT count(*) INTO v_n FROM pg_policies WHERE schemaname='public' AND tablename='school_income_records';
  IF v_n <> 2 THEN
    RAISE EXCEPTION 'PTWP_POST_INC_POLICY_COUNT: school_income_records 上有 % 条策略（期望 2）', v_n;
  END IF;
  ...
  SELECT c.relrowsecurity, c.relforcerowsecurity, coalesce(c.relacl::text,'')
    ...
    FROM pg_class c WHERE c.oid='public.school_personal_cash_income_linkage_events'::regclass;
  IF v_rls IS NOT TRUE OR v_force IS NOT FALSE THEN
    RAISE EXCEPTION 'PTWP_POST_EV_RLS: school_personal_cash_income_linkage_events rowsecurity=% force=%（期望 t / f）', v_rls, v_force;
  END IF;
  IF v_acl <> '{postgres=arwdDxtm/postgres,authenticated=rm/postgres,service_role=arwdDxtm/postgres}' THEN
    RAISE EXCEPTION 'PTWP_POST_EV_ACL: school_personal_cash_income_linkage_events 的授权为 %，期望 %（本脚本不改它的授权）', v_acl, '{postgres=arwdDxtm/postgres,authenticated=rm/postgres,service_role=arwdDxtm/postgres}';
  END IF;
  SELECT count(*) INTO v_n FROM pg_policies WHERE schemaname='public' AND tablename='school_personal_cash_income_linkage_events';
  IF v_n <> 1 THEN
    RAISE EXCEPTION 'PTWP_POST_EV_POLICY_COUNT: school_personal_cash_income_linkage_events 上有 % 条策略（期望 1）', v_n;
  END IF;
  ...
  WHERE (membership.is_active AND (membership.role = ''admin''::text)))))' THEN
    RAISE EXCEPTION 'PTWP_POST_INC_POLICY: school_restrict_part_time_work_income 的形状或表达式与预期不符：% / % / % / %',
  ...
  WHERE (i.id = school_personal_cash_income_linkage_events.income_record_id))) OR (EXISTS ( SELECT 1
   FROM school_get_current_app_membership() membership(user_id, role, is_active)
  WHERE (membership.is_active AND (membership.role = ''admin''::text)))))' THEN
    RAISE EXCEPTION 'PTWP_POST_EV_POLICY: school_select_income_linkage_events_by_visible_parent 的形状或表达式与预期不符：% / % / % / %',
      p.permissive, p.cmd, p.roles::text, p.qual;
  END IF;

  RAISE NOTICE 'PTWP_POST: 两张表的 RLS 开关、授权与策略集合符合预期';
END
$postp$;
