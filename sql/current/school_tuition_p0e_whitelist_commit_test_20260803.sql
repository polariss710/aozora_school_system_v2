-- P0-E whitelist commit test against the committed fixed whitelist fixture.
...
begin
  ...
  select * into strict v_void from public.school_void_atomic_student_tuition_generation_local(
    v_revision1,v_bill1,v_income1,v_manifest1,v_marker||' committed Void');
  begin
    perform * from public.school_relock_student_monthly_settlement(v_settlement,v_marker);
  ...
  select * into strict v_preview from public.school_get_atomic_tuition_reissue_preview_p0e(
    ...
    'neutralize_historical_carryover_v1',v_marker||' committed adjustment');
  ...
  from public.school_reissue_atomic_student_tuition_generation_p0e_local(
    ...
    v_preview.final_billing_amount_cny,v_preview.reason,v_marker||' committed Reissue');
...
select 'P0E_WHITELIST_COMMIT_TEST_PASSED' result;
commit;
