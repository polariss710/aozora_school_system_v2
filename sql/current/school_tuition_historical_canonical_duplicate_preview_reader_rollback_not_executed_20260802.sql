-- School V2 historical canonical duplicate-preview reader targeted rollback.
-- Rehearsal requires historical_canonical_preview_rollback_commit=0.
-- Formal rollback requires historical_canonical_preview_rollback_commit=1 and fresh business-owner authorization.
\set ON_ERROR_STOP on
\pset pager off

\if :{?historical_canonical_preview_rollback_commit}
\else
  \echo 'HISTORICAL_CANONICAL_PREVIEW_ROLLBACK_VARIABLE_REQUIRED'
  \quit
\endif
...
BEGIN
  IF md5(pg_get_functiondef(
     ...
     ))<>'6c5f083d2151f43d48bcb3b7d0cc9dfe' THEN
    RAISE EXCEPTION 'HISTORICAL_CANONICAL_PREVIEW_ROLLBACK_BASELINE_DRIFT';
  END IF;
  IF (SELECT count(*) FROM public.school_feature_gates
    ...
    RAISE EXCEPTION 'HISTORICAL_CANONICAL_PREVIEW_ROLLBACK_GATE_DRIFT';
  END IF;
END
$preflight$;
...
BEGIN
  IF md5(pg_get_functiondef(
    ...
    RAISE EXCEPTION 'HISTORICAL_CANONICAL_PREVIEW_ROLLBACK_VERIFY_FAILED';
  END IF;
END
$verify$;
...
\if :historical_canonical_preview_rollback_commit
  COMMIT;
  \echo 'HISTORICAL_CANONICAL_PREVIEW_READER_ROLLBACK_COMMITTED'
\else
  ROLLBACK;
  \echo 'HISTORICAL_CANONICAL_PREVIEW_READER_ROLLBACK_REHEARSED_NOT_EXECUTED'
\endif
