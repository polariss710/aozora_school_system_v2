...
select clock_timestamp() as session_b_started,:'p0d_scenario' scenario;
do $b$
declare
  v_scenario text:=current_setting('tuition.p0d_scenario');
...
begin
  ...
  if v_scenario in ('void_vs_void','void_vs_generate','void_vs_cash_reservation') then
    if v_scenario='void_vs_void' then
      perform * from public.school_void_atomic_student_tuition_generation_local(
        'd0d00000-0000-4000-8000-000000004001','d0d00000-0000-4000-8000-000000006001',
        'd0d00000-0000-4000-8000-000000007101',v_manifest,'codex-test concurrency');
      ...
      perform * from public.school_generate_student_tuition_bill_atomic_core(
        'd0d00000-0000-4000-8000-00000000a001','2020-08',0.042,v_manifest,'codex-test concurrency',null);
    else
      ...
      perform * from public.school_request_cash_income_confirmation_for_record(
        'd0d00000-0000-4000-8000-000000007101','d0d00000-0000-4000-8000-000000009001',
    ...
    if v_scenario='reissue_vs_cash_reservation' then
      ...
      begin
        perform * from public.school_unlock_student_monthly_settlement(
          'd0d00000-0000-4000-8000-00000000b001','codex-test concurrency');
      ...
      update public.school_feature_gates set state='enabled' where feature_key='student_tuition_generate';
      perform * from public.school_generate_student_tuition_bill_atomic_core(
        'd0d00000-0000-4000-8000-00000000a001','2020-08',0.042,v_manifest,'codex-test duplicate',null);
    end if;
    ...
    begin
      perform * from public.school_get_student_tuition_validation_preview_details(
        'b17abc58-2f64-4bad-bf20-c9643ead60bc','2026-07',0.042);
  ...
  else raise exception 'P0D_SCENARIO_INVALID: %',v_scenario;
  end if;
end;
$b$;
select clock_timestamp() as session_b_finished,:'p0d_scenario' scenario;
rollback;
