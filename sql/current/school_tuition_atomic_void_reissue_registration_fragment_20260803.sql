...
begin
  ...
  for r in select m.*,i.student_id,s.business_entity_id,i.billing_month,i.source,
                  i.canonical_bill_id,b.income_record_id,i.evidence,b.source_snapshot,inc.source_snapshot
  ...
  loop
    ...
    if r.manifest_kind='historical_registration_v1' then
      ...
      v_actual:=coalesce(r.evidence->>'generation_manifest_sha256',
                         r.source_snapshot->>'generation_manifest_sha256');
      if v_actual is distinct from r.inc.source_snapshot->>'generation_manifest_sha256' then
