-- Included by school_tuition_atomic_void_reissue_migration_20260803.sql.
...
begin
  if tg_op='DELETE' then raise exception 'TUITION_GENERATION_REVISION_DELETE_FORBIDDEN'; end if;
  if tg_op='UPDATE' then
    ...
    if not v_context_ok or old.lifecycle_status<>'active' or new.lifecycle_status<>'voided'
       or (to_jsonb(new)-'lifecycle_status') is distinct from (to_jsonb(old)-'lifecycle_status') then
