...
-- ---------------------------------------------------------------------------
-- §7 负例（设计 §11.2 场景 4 的可做部分）
--
--   ⚠️ 原方案（直接 UPDATE 刚生成的行再跑 validator）**已作废**。
--   09-06 18:39 审查查明生产有两道 BEFORE 守卫：
--     · school_guard_r0_tuition_business_mutation()（bills / income_records）
--       只在 school_tuition_atomic_writer_context 有当前 backend_pid + txid
--       的记录时放行；而 writer 在结束时**自己把那行删掉了**，
--       所以生成之后的任何直接 UPDATE 都会撞
--       TUITION_GENERATION_BLOCKED / TUITION_DIRECT_DML_FORBIDDEN
--     · school_guard_tuition_identity_or_lesson_immutable()（bill_lessons）
--       BEFORE DELETE OR UPDATE 无条件抛 TUITION_IMMUTABLE_ROW，
--       没有「本事务新行」例外
--   原来那十条篡改型负例**一条也到不了 validator**。
--
--   我的错在于把「业务负责人允许我 UPDATE」当成了「数据库允许我 UPDATE」。
--   授权是人给的，守卫是系统的，两者无关。
--   **不注入 writer_context、不停用守卫、不动 gate。**
--
--   新方案：**根本不篡改**。用已审过的 v1 / v2 函数版本**混搭**，
--   让 writer 沿完全合法的路径写出失配状态，由 validator 自己拒绝。
--   零直接 DML、零合成行、零守卫绕过。
--
--   两个用例都落在 7aef8061 的 09 / 10 月
--   （R2 实测无 active 账单；O1 实测 v1/v2 候选集同为 24 条；
--     R1 实测 v2 序号和 28）：
--     · v1 builder + v2 writer → 冻结 total=28，v2 分支要 count=24 → 失配
--     · v2 builder + v1 writer → 冻结 total=24，缺键走 v1 分支要 sum=28 → 失配
--   两条都必须抛 TUITION_ATOMIC_BILL_LESSON_MISMATCH。
--   **这就是「版本分支真的在切算法」的证据**：任一方向错配都被抓住。
--
--   ⚠️ 本轮**做不到**的负例，如实记录：
--     未知版本值 / JSON null / 非字符串 / bill-income 版本不一致。
--     它们需要写出坏数据，而生产守卫（正确地）不允许任何人写坏数据。
--     端到端验证只能靠「故意写坏的 writer 变体」，那是新的授权范围。
--     目前这几条只有表达式层证据（只读脚本 T7，7 种输入 7/7）。
-- ---------------------------------------------------------------------------
...
-- 还原 public.school_build_student_tuition_generation_snapshot(uuid,text,numeric) 为 v1
CREATE OR REPLACE FUNCTION public.school_build_student_tuition_generation_snapshot(p_student_id uuid, p_billing_month text, p_billing_exchange_rate numeric)
 RETURNS TABLE(student_id uuid, business_entity_id uuid, billing_month text, previous_settlement_month text, previous_settlement_id uuid, previous_carryover_cny numeric, carryover_evidence jsonb, candidate_count integer, total_lesson_count integer, total_duration_hours numeric, total_base_lesson_fee_jpy numeric, total_aircon_fee_jpy numeric, total_fee_jpy numeric, billing_exchange_rate numeric, billing_amount_cny numeric, candidate_uuid_md5 text, candidate_manifest_sha256 text, generation_manifest_sha256 text, candidates jsonb)
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
...
DO $do$
DECLARE s record; v_res record; v_rate numeric;
BEGIN
  SELECT rate INTO v_rate FROM lc2w_run;
  ...
  EXCEPTION WHEN OTHERS THEN
    ...
    VALUES ('mix_v1builder_v2writer','builder 回 v1（total=源序号和 28）+ writer 仍标 v2（validator 按条数 24 校验）','TUITION_ATOMIC_BILL_LESSON_MISMATCH',
            SQLERRM NOT LIKE 'LC2W_SENTINEL_NOT_REJECTED%', SQLSTATE, SQLERRM);
  END;
END
$do$;
...
-- 还原 public.school_generate_student_tuition_bill_atomic_base_core_v1(uuid,text,numeric,text,text,text) 为 v1
CREATE OR REPLACE FUNCTION public.school_generate_student_tuition_bill_atomic_base_core_v1(p_student_id uuid, p_billing_month text, p_billing_exchange_rate numeric, p_expected_generation_manifest_sha256 text, p_note text DEFAULT NULL::text, p_test_fail_after_step text DEFAULT NULL::text)
 RETURNS TABLE(tuition_bill_id uuid, billing_identity_id uuid, income_record_id uuid, student_id uuid, business_entity_id uuid, billing_month text, generation_manifest_sha256 text, candidate_count integer, total_lesson_count integer, total_duration_hours numeric, total_base_lesson_fee_jpy numeric, total_aircon_fee_jpy numeric, total_fee_jpy numeric, billing_exchange_rate numeric, previous_carryover_cny numeric, billing_amount_cny numeric, bill_status text, income_status text, idempotent boolean, message text)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
-- 还原 public.school_generate_student_tuition_next_revision_core(uuid,uuid,uuid,text,numeric,text,text,text) 为 v1
CREATE OR REPLACE FUNCTION public.school_generate_student_tuition_next_revision_core(p_generation_identity_id uuid, p_previous_revision_id uuid, p_student_id uuid, p_billing_month text, p_billing_exchange_rate numeric, p_expected_generation_manifest_sha256 text, p_note text, p_test_fail_after_step text)
 RETURNS TABLE(tuition_bill_id uuid, billing_identity_id uuid, income_record_id uuid, student_id uuid, business_entity_id uuid, billing_month text, generation_manifest_sha256 text, candidate_count integer, total_lesson_count integer, total_duration_hours numeric, total_base_lesson_fee_jpy numeric, total_aircon_fee_jpy numeric, total_fee_jpy numeric, billing_exchange_rate numeric, previous_carryover_cny numeric, billing_amount_cny numeric, bill_status text, income_status text, idempotent boolean, message text)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
-- 还原 public.school_generate_student_tuition_next_revision_p0e_core(uuid,uuid,uuid,text,numeric,text,text,text) 为 v1
CREATE OR REPLACE FUNCTION public.school_generate_student_tuition_next_revision_p0e_core(p_generation_identity_id uuid, p_previous_revision_id uuid, p_student_id uuid, p_billing_month text, p_billing_exchange_rate numeric, p_expected_generation_manifest_sha256 text, p_note text, p_test_fail_after_step text)
 RETURNS TABLE(tuition_bill_id uuid, billing_identity_id uuid, income_record_id uuid, student_id uuid, business_entity_id uuid, billing_month text, generation_manifest_sha256 text, candidate_count integer, total_lesson_count integer, total_duration_hours numeric, total_base_lesson_fee_jpy numeric, total_aircon_fee_jpy numeric, total_fee_jpy numeric, billing_exchange_rate numeric, previous_carryover_cny numeric, billing_amount_cny numeric, bill_status text, income_status text, idempotent boolean, message text)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    INSERT INTO lc2w_neg(case_name,description,expect_needle,rejected,sqlstate,err)
    VALUES ('mix_v2builder_v1writer','builder 用 v2（total=条数 24）+ writer 回 v1 不标版本键（validator 按序号和 28 校验）','TUITION_ATOMIC_BILL_LESSON_MISMATCH',
            SQLERRM NOT LIKE 'LC2W_SENTINEL_NOT_REJECTED%', SQLSTATE, SQLERRM);
  END;
END
$do$;
...
-- 恢复 public.school_generate_student_tuition_next_revision_core(uuid,uuid,uuid,text,numeric,text,text,text) 为 v2
CREATE OR REPLACE FUNCTION public.school_generate_student_tuition_next_revision_core(p_generation_identity_id uuid, p_previous_revision_id uuid, p_student_id uuid, p_billing_month text, p_billing_exchange_rate numeric, p_expected_generation_manifest_sha256 text, p_note text, p_test_fail_after_step text)
 RETURNS TABLE(tuition_bill_id uuid, billing_identity_id uuid, income_record_id uuid, student_id uuid, business_entity_id uuid, billing_month text, generation_manifest_sha256 text, candidate_count integer, total_lesson_count integer, total_duration_hours numeric, total_base_lesson_fee_jpy numeric, total_aircon_fee_jpy numeric, total_fee_jpy numeric, billing_exchange_rate numeric, previous_carryover_cny numeric, billing_amount_cny numeric, bill_status text, income_status text, idempotent boolean, message text)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
-- 恢复 public.school_generate_student_tuition_next_revision_p0e_core(uuid,uuid,uuid,text,numeric,text,text,text) 为 v2
CREATE OR REPLACE FUNCTION public.school_generate_student_tuition_next_revision_p0e_core(p_generation_identity_id uuid, p_previous_revision_id uuid, p_student_id uuid, p_billing_month text, p_billing_exchange_rate numeric, p_expected_generation_manifest_sha256 text, p_note text, p_test_fail_after_step text)
 RETURNS TABLE(tuition_bill_id uuid, billing_identity_id uuid, income_record_id uuid, student_id uuid, business_entity_id uuid, billing_month text, generation_manifest_sha256 text, candidate_count integer, total_lesson_count integer, total_duration_hours numeric, total_base_lesson_fee_jpy numeric, total_aircon_fee_jpy numeric, total_fee_jpy numeric, billing_exchange_rate numeric, previous_carryover_cny numeric, billing_amount_cny numeric, bill_status text, income_status text, idempotent boolean, message text)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
\echo '--- 负例结果 ---'
SELECT case_name AS "用例", description AS "构造方式",
       rejected AS "被拒绝", sqlstate AS "SQLSTATE", left(err,70) AS "实际错误"
...
BEGIN
  ...
  IF coalesce(v_bad,'') <> '' THEN
    RAISE EXCEPTION 'LC2W_N1_NOT_REJECTED: 以下版本错配**没有**被拒绝，'
      '说明版本分支没起作用。%', v_bad;
  END IF;
END
$do$;
...
BEGIN
  SELECT string_agg(format(E'\n  %s: 期望含 %s，实际 %s',
           case_name, expect_needle, left(err,90)), '')
...
-- (N3) 负例的失败必须已被子事务回滚干净：三张正例账单重新全量校验
DO $do$
DECLARE t record;
BEGIN
  FOR t IN SELECT * FROM lc2w_target ORDER BY label LOOP
...
\echo '>>> §7 版本错配全部按预期被拒绝，且未留下任何行'

-- ---------------------------------------------------------------------------
-- §8 收尾：无条件 ROLLBACK
-- ---------------------------------------------------------------------------
\echo '--- writer 上下文表（应为 0）---'
SELECT count(*) AS "context 行数" FROM public.school_tuition_atomic_writer_context;

ROLLBACK;

\echo ''
\echo '>>> ROLLBACK 已执行：五次生成尝试（3 成功 + 2 预期失败）全部撤销。'
