-- Isolated PostgreSQL bootstrap for School V2 Phase 2I-A.
...
create table public.school_personal_cash_income_linkage_events(
  ...
  cash_request_id uuid,cash_transaction_id uuid,sync_status text
);

create function public.school_assert_active_lesson_writer()
returns void language plpgsql security definer
set search_path=pg_catalog,public as $function$
declare v_actor uuid:=auth.uid();v_role text;v_active boolean;
begin
  if v_actor is null then raise exception 'LESSON_WRITER_AUTH_REQUIRED'; end if;
  ...
  if not found or v_active is distinct from true
     or v_role not in ('admin','operator') then
    raise exception 'LESSON_WRITER_MEMBERSHIP_REQUIRED';
  end if;
end
$function$;
...
begin
  ...
  return query select row_value.* from public.school_lesson_records row_value
    where row_value.id=v_id;
end
$function$;
...
insert into public.school_personal_cash_income_linkage_events values
  ('9de972ff-8e66-470a-8b05-e430ef51562f','91756564-c48d-4a1d-b6bc-88a041660e46',
   'a0bee5be-761b-4bc0-a666-411f033e1eba','f500dbe4-07a9-4a4d-ac99-e68592a8af6a','synced');
