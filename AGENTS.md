...
- Read-only DB verification commands such as `select` from `information_schema`, `pg_constraint`, `pg_indexes`, `pg_description`, `pg_proc`, and row-count checks may use "Yes, and don't ask again" when the command is clearly read-only.
