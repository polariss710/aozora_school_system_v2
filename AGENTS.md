...
- For write-operation SQL/RPC work, use the full autopilot workflow by default after the Schema And Business Model Expansion Gate passes: analysis, DB verification, schema/RPC design, SQL draft, static review, SQL/RPC execution, rollback test, whitelist commit test, verified SQL commit, frontend implementation, checkpoint, current-status update, commit, and push.
...
- This gate overrides phase-level authorization, full autopilot, and general wording in a task. Phrases such as "modify necessary schema", "make required DB changes", "implement the complete feature", "full autopilot", "design and implement autonomously", or "use the best approach" do not authorize business-model expansion.
...
- An approval is valid only when a current-task instruction or an explicitly referenced still-applicable business-owner decision identifies the exact object, the exact new or changed semantics, and the intended authority/mutability/locking or compatibility behavior. Silence, lack of objection, an implementation proposal, an already-created unapproved object, previous accidental deployment, or blanket wording is not approval. Each non-`none` declaration item must map separately to its approval; one general reference is insufficient unless it explicitly enumerates every item.
- Business-model expansion includes any new business table; business column; enum or status value; date, month, attribution, identity, source, snapshot, version, writable-fact, or authoritative-source concept; or any change to an existing field's business meaning, mutability, ownership, writer, reader precedence, or locking rule. Promoting a diagnostic field to production authority, reinterpreting historical data, adding a long-lived legacy path, adding dual writes, fallback, `COALESCE`, or NULL-based branching to decide a business fact, and destructive schema changes such as drop, rename, type change, or semantic repurposing are also expansions.
- Every business fact must have exactly one declared authoritative source. Two writable fields must not express the same fact; different readers must not select different authorities; a writer must not write one authority while readers use another; frontend code must not recreate a DB-authoritative fact. An immutable snapshot may preserve a frozen historical fact only when its contract clearly distinguishes it from current operational authority.
- When old and new fields must coexist during an explicitly approved migration, the approval and implementation must define the sole authority, deterministic migration direction and reader precedence, every writer and reader, monitoring, compatibility deadline, completion criteria, retirement condition, and historical-data impact. Legacy fields may remain for audit, diagnostics, or migration evidence but must not decide production business without such a contract. A compatibility path without a retirement condition must not be implemented autonomously.
- Indexes, foreign keys, check or unique constraints, triggers, functions/RPCs, views, ACL/RLS rules, comments, test fixtures, rollback tests, postdeploy checks, and performance optimizations may proceed under normal phase-level full autopilot without separate object-by-object approval only when they enforce an already approved model and introduce no business fact, authority, permission boundary, writer/reader priority, or historical interpretation beyond that approved model.
- Before any design or implementation task proceeds beyond read-only investigation, complete the Business-model expansion declaration required by `docs/workflows/write-rpc-flow.md`. Every item must name exact objects or say `none`. Any non-`none` item requires a current-task approval reference that explicitly approves that object and its semantics.
- If implementation discovers an unapproved expansion, stop immediately before drafting or changing business code/SQL. Preserve read-only findings, explain why the existing model is insufficient, provide the exact proposed object and impact contract, and wait for explicit business-owner approval. Do not create a temporary/compatibility field, fallback, dual write, or migration first and seek approval afterward.
- A missing, vague, or unmatched approval reference for any expansion is a hard stop. Existing phase authorization must never be treated as the missing approval.
...
- During full autopilot, schema execution, RPC execution, rollback tests, whitelisted commit tests, `commit`, and `push` are phase-authorized by the initial prompt when the workflow phase requires them, the Schema And Business Model Expansion Gate has passed, and all required checks have passed.
...
- Do not draft or execute an unapproved business-model expansion. General phase authorization and full autopilot never substitute for the explicit approval required by the Schema And Business Model Expansion Gate.
