...
- 当前状态: V1 可用。支付列表可确认支付、反转已支付请求、取消 pending、恢复 cancelled、reissue reversed；详情页只读。个人业务 Cash System linkage Phase 1 已验证：Cash-side external JPY RPC、school-side mapping/outbox schema/RPC/API、payment confirm 到 school pending outbox、手动同步器写入 Cash JPY transaction 并回写 school synced。Cash linkage v2 的 School 侧 request lifecycle SQL/RPC 已 apply 并通过 ROLLBACK 验证；Edge Function 桥接代码已加入 `supabase/functions/request-cash-confirmation/`，但尚未部署、尚未接入页面，Cash approve/reject 后回写 School 仍未实现。该手动同步器是验证/运维工具，不是最终日常业务入口。
- 最近关键更新: 2026-06-13 新增 Edge Function `request-cash-confirmation`。它接收 `payment_request_id`、`cash_account_mapping_id`、可选 `note`，按顺序调用 School request RPC、Cash pending request RPC、School submitted RPC，把 School event 从 `pending_cash_request` 推进到 `awaiting_cash_confirmation`。它只创建 Cash pending request，不创建 Cash transaction，不把 payment request 改为 `paid`。此前新增并 apply 的 `school_personal_cash_payment_confirmation_lifecycle_schema.sql` 和 `school_request_personal_cash_payment_confirmation_rpc.sql` 已验证 request RPC 不写 `paid_at`、school expense、school account transaction 或 Cash transaction。旧脚本只读取 `sync_status = pending` 且已 paid 的 eligible outbox，因此不会处理 v2 `pending_cash_request` / `awaiting_cash_confirmation` 事件。
- 当前限制 / hard stop: 不删除 payment request、wage lock、expense、account transaction。青空塾确认支付路径必须保持现有 school account/expense/account transaction 行为。最终 Cash linkage v2 目标不是 School 直接创建 Cash transaction，而是 School 页面请求同步、Cash pending request、Cash 页面 approve/reject；approve 后才生成 Cash transaction 并改变 Cash 余额，reject 后不生成 Cash transaction 且 School 显示 cash_rejected 或保持未支付。继续排除 Cash CNY、报销、法人账户、学费收入、兼职工资、撤销同步、自动后台任务、跨 DB 强事务或历史 backfill。已 synced 的个人业务支付若要撤销，应继续阻断，直到 reversal sync 独立实现。
- 下一步: 未来改 payment status actions 时，显式重测 cancel/restore/reissue 和 confirm/reverse 链路；如需真正开放支付记录编辑，必须先设计独立 edit RPC/API guard。personal Cash linkage v2 后续阶段是 Edge Function 部署/配置、School payment UI 调整、Cash approve/reject 后的 School confirmed/rejected 回写 RPC、5 月两笔 JPY 老师工资 approve/reject 试运行；仍不做跨 DB 强事务。
