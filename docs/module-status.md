...
- 当前状态: V1 可用。支付列表可确认支付、反转已支付请求、取消 pending、恢复 cancelled、reissue reversed；详情页只读。个人业务 Cash System linkage Phase 1 已验证：Cash-side external JPY RPC、school-side mapping/outbox schema/RPC/API、payment confirm 到 school pending outbox、手动同步器写入 Cash JPY transaction 并回写 school synced。Cash linkage v2 的 School 侧 request lifecycle SQL/RPC 已 apply 并通过 ROLLBACK 验证；老师工资支付页已完成第一段 School-only 嵌入式请求入口：personal + `teacher_wage` + JPY 的 pending 请求选择 Cash 支付账户后调用 `school_request_personal_cash_payment_confirmation(...)`，只生成/复用 School `pending_cash_request` linkage event，不把 payment request 改为 `paid`。Edge Function 桥接代码已加入 `supabase/functions/request-cash-confirmation/`，当前只支持 personal `teacher_wage` JPY payment request，不接收入页；但尚未部署、尚未接入页面，Cash pending request 创建和 approve/reject 后回写 School 仍未实现。该手动同步器是验证/运维工具，不是最终日常业务入口。
