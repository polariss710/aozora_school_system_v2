...
| 老师工资支付 | V1 可用 + personal Cash mapping/outbox ready | Retest status actions before future changes; personal-business Cash payment-confirm integration requires a separate guarded phase |
...
- 当前状态: V1 可用。支付列表可确认支付、反转已支付请求、取消 pending、恢复 cancelled、reissue reversed；详情页只读。个人业务 Cash System linkage 已完成 Cash-side external JPY RPC 和 school-side mapping/outbox schema/RPC/API，但尚未接入支付确认 UI/API/RPC。
- 最近关键更新: 2026-06-13 完成 school-side Phase 1 mapping/outbox 最小补强：执行 `school_personal_cash_linkage_schema.sql` 新增 `school_personal_cash_account_mappings` / `school_personal_cash_linkage_events`；执行 `school_personal_cash_linkage_rpcs.sql` 新增 mapping create/update/list、linkage event create/query、sync status update RPC；新增 `js/api/personal-cash-linkage-api.js`。Rollback 验证 personal-only guard、重复 event 幂等、sync status 更新，残留 0；白名单 commit test 留存 `codex-test` school 测试数据：business entity `92000000-0000-4000-8000-000000132001`、payment request `92000000-0000-4000-8000-000000132101`、mapping `f5c02610-1b11-4353-b5de-ae5b3b60f980`、pending event `9b95e09a-09c4-4203-bc02-07daaf1beb5b`。未写 Cash DB，未改支付确认主流程。此前同日完成 Cash-side `home_create_external_jpy_transaction` 与 external/idempotency metadata。
- 当前限制 / hard stop: 不删除 payment request、wage lock、expense、account transaction。确认支付必须不创建 reimbursement/income/student settlement/wage/lesson 等非本链路数据。Cash linkage 的 payment confirmation UI/API/RPC 接入尚未实装；青空塾、报销、法人账户、CNY、学费收入、兼职工资、跨 DB 强事务、自动重试后台任务、撤销同步、历史 backfill 均不在当前 Phase 1 已完成范围。
- 下一步: 未来改 payment status actions 时，显式重测 cancel/restore/reissue 和 confirm/reverse 链路；如需真正开放支付记录编辑，必须先设计独立 edit RPC/API guard，不能绕过现有支付确认/反转链路。personal Cash linkage 下一阶段是 payment confirmation UI/API/RPC 接入：按 `entity_type = company` 走 legacy school path，按 `entity_type = personal` + teacher_wage + JPY 选择 active Cash mapping 并创建 pending outbox，再调用 Cash external JPY RPC；仍需 rollback/白名单端到端验证。
