...
- 最近关键更新: 2026-06-16 经 dry-run、rollback 验证和用户确认，已删除历史账户 `吴个人结算账户日元` 及其 School-side 旧财务链：8 条 income、15 条 expense、15 条 payment request，并随账户删除级联清理 28 条 account transaction；课时记录、老师工资 lock/detail、正式账户 `GMOあおぞらネット銀行` / `包垫付金额` / `吴垫付金额`、2026-05 诺应教育 canonical 收入和 2026-05 teacher_wage canonical expense records 均保留。2026-06-15 账户管理筛选已精简，移除“流水类型”筛选但保留账户列表和流水展示。此前数据清理已删除 `吴个人结算账户人民币` 及其 1 条 adjustment、8 条历史/测试账户流水，删除后账户 orphan reference 为 0。2026-06-13 增加 `school_create_account_profile(..., p_app_type)` 和 `school_update_account_profile(..., p_app_type)` 重载，保留旧重载。账户页新增“账户用途”筛选和 create/edit 用途字段；`school` / `family` 可新增/编辑，`store` 本阶段只展示/筛选不编辑。Family 账户强制 `business_entity_id = NULL`、`is_company_account = false`，资料编辑不能改 `app_type`，新增/编辑不创建账户流水。
