...
| 老师工资支付 | V1 可用 + personal Cash payment outbox ready | Retest status actions before future changes; cross-project Cash write/retry integration requires a separate guarded phase |
...
- 当前状态: V1 可用。支付列表可确认支付、反转已支付请求、取消 pending、恢复 cancelled、reissue reversed；详情页只读。个人业务 Cash System linkage 已完成 Cash-side external JPY RPC、school-side mapping/outbox schema/RPC/API，以及 payment confirm UI/API/RPC 到 school pending outbox 的接入；尚未写 Cash DB。
- 最近关键更新: 2026-06-13 完成 payment-confirm outbox 接入：执行 `school_confirm_personal_cash_payment_request_rpc.sql`，个人业务 `teacher_wage` JPY pending payment 通过 Cash mapping 确认后只在 school DB 标记 paid 并创建 pending outbox，不创建 school expense/account transaction/account balance side effect，不写 Cash DB。支付页按业务归属切换 selector：company / 青空塾 仍走 school account 和原 `school_confirm_payment_request`；personal + teacher_wage + JPY 显示 active Cash mapping，缺少映射时阻断。详情页新增 Cash linkage 只读状态。Rollback 验证残留 0；白名单 commit test 留存 `codex-test` school 测试数据：business entity `93000000-0000-4000-8000-000000141001`、payment request `93000000-0000-4000-8000-000000141101`、mapping `eaf3b59d-f944-441f-911f-b639ba284c78`、pending event `11f8f9ee-cbf4-4a29-8d6a-dc56a7d2e7e4`。
- 当前限制 / hard stop: 不删除 payment request、wage lock、expense、account transaction。青空塾确认支付路径必须保持现有 school account/expense/account transaction 行为。个人 Cash payment outbox 不做 Cash DB 写入、自动重试、撤销同步、CNY、学费收入、兼职工资、报销、法人账户或历史 backfill。已有 Cash outbox event 的 paid payment 在页面上不开放撤销，避免本阶段没有反向 Cash 同步时分叉。
- 下一步: 未来改 payment status actions 时，显式重测 cancel/restore/reissue 和 confirm/reverse 链路；如需真正开放支付记录编辑，必须先设计独立 edit RPC/API guard。personal Cash linkage 下一阶段是跨项目 Cash write/retry integration：读取 pending school outbox，调用 Cash `home_create_external_jpy_transaction`，成功后回写 `cash_transaction_id` / `sync_status = synced`；仍不做跨 DB 强事务。
