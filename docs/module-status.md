...
- 当前状态: V1 可用。支付列表可确认支付、反转已支付请求、取消 pending、恢复 cancelled、reissue reversed；详情页只读。个人业务 Cash System linkage Phase 1 已验证：Cash-side external JPY RPC、school-side mapping/outbox schema/RPC/API、payment confirm 到 school pending outbox、手动同步器写入 Cash JPY transaction 并回写 school synced。Cash linkage v2 的 School 侧 request lifecycle SQL/RPC 已 apply 并通过 ROLLBACK 验证；老师工资支付页 personal + `teacher_wage` + JPY 的 pending 请求选择 Cash 支付账户后，通过 `js/api/payment-api.js` 的 `requestCashConfirmationViaFunction(...)` 调用 `request-cash-confirmation` Edge Function。该 Function 创建/复用 School linkage event，创建 Cash pending request，并把 School event 移到 `awaiting_cash_confirmation`；不把 payment request 改为 `paid`。Function 当前只支持 personal `teacher_wage` JPY payment request，不接收入页；尚未部署、尚未运行，Cash approve/reject 后回写 School 仍未实现。该手动同步器是验证/运维工具，不是最终日常业务入口。
- 最近关键更新: 2026-06-13 修正 v2 UI 方向并接入 Edge Function wrapper：不新增独立 School 同步入口。personal + `teacher_wage` + JPY 的支付动作在老师工资支付页面内选择 Cash 支付账户，按钮文案使用 `提交到 Cash 确认`，提交后调用 `request-cash-confirmation` 并提示 `已提交到 Cash System 待确认`。Cash System 确认后才会记账并完成支付，拒绝时不会改变 Cash 余额且 School 侧仍保持未支付。本轮只接页面/API 调用，不部署 Function、不运行测试、不实现 Cash approve/reject 回写。
...
- 下一步: 未来改 payment status actions 时，显式重测 cancel/restore/reissue 和 confirm/reverse 链路；如需真正开放支付记录编辑，必须先设计独立 edit RPC/API guard。personal Cash linkage v2 后续阶段是 Edge Function 部署/配置、Cash approve/reject 后的 School confirmed/rejected 回写 RPC、5 月两笔 JPY 老师工资 approve/reject 试运行；仍不做跨 DB 强事务。
