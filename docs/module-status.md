...
- 后续优先级: keep planned-only import and cross-month makeup completion as stable V1 surfaces; keep full actual/history migration import out of current implementation; design void restore and wage-lock generation only as separate guarded phases.
...
- 后续优先级: first preserve completed V1 surfaces; then handle full actual import/history migration, wage lock generation, and other backlog as separate phases.
