...
| 收入记录 | V1 可用 + Phase 2 personal tuition Cash linkage E2E complete + sync status UI | Tuition reverse sync, automatic sync scheduling, and failed retry UI remain separate phases |
...
- 下一步: Tuition income reverse sync、自动调度 sync executor、failed retry 操作入口、部分收款、付款计划、旧流水修正、跨账户收入迁移、结算外费用是否进入学生账单，均需另开 guarded design。
