...
| 收入记录 | V1 可用 + Phase 2 tuition Cash DB/create RPC/sync/UI/detail guard code prepared | Guarded SQL execution, controlled UI/create verification, and controlled sync verification remain separate steps |
...
- 当前状态: V1 可用。支付列表可确认支付、反转已支付请求、取消 pending、恢复 cancelled、reissue reversed；详情页只读。个人业务 Cash System linkage 已完成 Phase 1 端到端：Cash-side external JPY RPC、school-side mapping/outbox schema/RPC/API、payment confirm 到 school pending outbox、手动同步器写入 Cash JPY transaction 并回写 school synced。
- 最近关键更新: 2026-06-13 完成手动同步器 `scripts/sync-personal-cash-linkage.zsh`：读取 eligible pending outbox，调用 Cash `home_create_external_jpy_transaction`，成功后通过 `school_update_personal_cash_linkage_event_status` 回写 `synced` 和 Cash transaction id，失败时回写 `failed` 和错误信息。白名单 E2E 成功测试：Cash account `94000000-0000-4000-8000-000000150501`、school payment request `94000000-0000-4000-8000-000000150101`、school event `2f20e264-1e20-493d-b92a-58c244abfa09`、Cash transaction `fbd3e5df-14be-4b3b-9a0b-319f4416968b`。重复执行未重复生成 Cash 流水；失败路径 event `11f8f9ee-cbf4-4a29-8d6a-dc56a7d2e7e4` 已进入 `failed`；旧 fake pending event `9b95e09a-09c4-4203-bc02-07daaf1beb5b` 已标记 `blocked`。
- 当前限制 / hard stop: 不删除 payment request、wage lock、expense、account transaction。青空塾确认支付路径必须保持现有 school account/expense/account transaction 行为。个人 Cash sync 只处理 personal + paid teacher_wage + JPY + pending outbox；不处理 Cash CNY、报销、法人账户、学费收入、兼职工资、撤销同步、自动后台任务、跨 DB 强事务或历史 backfill。已 synced 的个人业务支付若要撤销，应继续阻断，直到 reversal sync 独立实现。
- 下一步: 未来改 payment status actions 时，显式重测 cancel/restore/reissue 和 confirm/reverse 链路；如需真正开放支付记录编辑，必须先设计独立 edit RPC/API guard。personal Cash linkage 后续阶段是 reversal sync、retry UI/操作面、测试数据清理或正式配置迁移；仍不做跨 DB 强事务。
...
- 最近关键更新: 2026-06-13 准备但未执行 Phase 2 personal-business tuition income -> Cash System JPY income 的 school SQL、sync code、income entry UI/API 和 detail guard：`school_personal_cash_income_linkage_schema.sql` 扩展 `school_personal_cash_account_mappings.flow_type = tuition_income` 并新增独立 `school_personal_cash_income_linkage_events` outbox；`school_personal_cash_income_linkage_rpcs.sql` 新增 `school_update_personal_cash_income_linkage_event_status` 用于 sync executor 回写 pending/synced/failed；`school_create_personal_cash_tuition_income_record_rpc.sql` 新增专用 create RPC，创建 `school_income_records` + pending income linkage event，但不写 school account balance 或 account transaction；`scripts/sync-personal-cash-linkage.zsh` 新增 income linkage 分支，调用 Cash RPC 创建 JPY `income` transaction 并回写 income event；`income.html` / `js/pages/income-page.js` / `js/api/income-api.js` 新增“个人业务学费收入（Cash System）”创建方式；`income-detail.html` / `js/api/income-detail-api.js` / `js/pages/income-detail-page.js` 识别 Cash-linked tuition income 并隐藏普通 edit/reverse；`school_update_income_record_rpc.sql` 和 `school_reverse_income_record_rpc.sql` 增加后端 guard，拒绝已进入 `tuition_income_received` linkage 的收入走普通 edit/reverse。2026-06-12 收入新增/编辑字段统一；账户币种不再作为 dialog 输入，改由入账账户决定并由 RPC 校验。
- 当前限制 / hard stop: 本轮 Phase 2 SQL 尚未执行；未连接 DB、不执行 SQL、不调用 RPC、不写测试数据。普通收入路径仍按 V1：非 Cash path 会写入收入记录、账户余额和账户流水；Phase 2 personal Cash path 必须保持 personal + tuition + JPY + Cash mapping guard，且不处理 青空塾、CNY、报销、法人账户、非学费收入或兼职工资。不从 account transaction 反推收入；不删除收入；不重算 locked student settlement。普通 edit/reverse 仅允许没有 Cash income linkage event 的普通收入；已进入 Cash linkage 的 tuition income 在 Phase 2 v1 中禁止普通 edit/reverse。普通收入编辑仍要求未撤销、非学生收款链路、未被锁定学生月结阻挡、原始 `income_adjust` 流水唯一且一致、且该流水仍是账户最新流水；不允许直接更换入账账户，旧流水或复杂账户链路应撤销后重新新增。Reversal 必须保留原收入和原交易。
- 下一步: 先执行/验证 Phase 2 DB foundation + create RPC SQL，再做 controlled UI/create verification 和 controlled sync verification。更复杂的部分收款、付款计划、旧流水修正、跨账户收入迁移、结算外费用是否进入学生账单、Cash-linked tuition reverse sync / edit workflow，需另开 guarded design。
