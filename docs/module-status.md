...
- 当前状态: V1 可用。支付列表可确认支付、反转已支付请求、取消 pending、恢复 cancelled、reissue reversed；详情页只读。个人业务 Cash System linkage Phase 1 已验证：Cash-side external JPY RPC、school-side mapping/outbox schema/RPC/API、payment confirm 到 school pending outbox、手动同步器写入 Cash JPY transaction 并回写 school synced。Cash linkage v2 的 School 侧 request lifecycle SQL/RPC 已准备：School 可以先创建 `pending_cash_request` linkage event，后续 Cash request id 回写后进入 `awaiting_cash_confirmation`，但 UI/Edge Function/Cash approve-reject 回写仍未实现。该手动同步器是验证/运维工具，不是最终日常业务入口。
- 最近关键更新: 2026-06-13 新增正式 SQL 草案 `school_personal_cash_payment_confirmation_lifecycle_schema.sql` 和 `school_request_personal_cash_payment_confirmation_rpc.sql`。前者扩展 `school_personal_cash_linkage_events` 的 Cash request snapshot 字段与 v2 状态；后者新增 `school_request_personal_cash_payment_confirmation(...)` 和 `school_mark_personal_cash_payment_request_submitted(...)`。request RPC 只创建/复用 School linkage event，不把 payment request 改为 `paid`，不写 `paid_at`，不创建 school expense / account transaction，也不创建 Cash transaction。此前已完成手动同步器 `scripts/sync-personal-cash-linkage.zsh` 运维说明：旧脚本只读取 `sync_status = pending` 且已 paid 的 eligible outbox，因此不会处理 v2 `pending_cash_request` / `awaiting_cash_confirmation` 事件。
...
- 下一步: 未来改 payment status actions 时，显式重测 cancel/restore/reissue 和 confirm/reverse 链路；如需真正开放支付记录编辑，必须先设计独立 edit RPC/API guard。personal Cash linkage v2 后续阶段是 apply/test School lifecycle SQL、Edge Function 桥接、School payment UI 调整、Cash approve/reject 后的 School confirmed/rejected 回写 RPC、5 月两笔 JPY 老师工资 approve/reject 试运行；仍不做跨 DB 强事务。
