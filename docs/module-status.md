...
| 老师工资支付 | V1 可用 + personal Cash Phase 1 verified; v2 pending-request confirmation planned; minimal School Auth added for Edge Function JWT | Page-driven Cash request/approve/reject E2E and full operator/admin permissions remain separate guarded phases |
...
- 当前状态: V1 可用。支付列表可确认支付、反转已支付请求、取消 pending、恢复 cancelled、reissue reversed；详情页只读。个人业务 Cash System linkage Phase 1 已验证：Cash-side external JPY RPC、school-side mapping/outbox schema/RPC/API、payment confirm 到 school pending outbox、手动同步器写入 Cash JPY transaction 并回写 school synced。Cash linkage v2 的 School 侧 request lifecycle SQL/RPC 已 apply 并通过 ROLLBACK 验证；老师工资支付页 personal + `teacher_wage` + JPY 的 pending 请求选择 Cash 支付账户后，通过 `js/api/payment-api.js` 的 `requestCashConfirmationViaFunction(...)` 调用 `request-cash-confirmation` Edge Function。该 Function 创建/复用 School linkage event，创建 Cash pending request，并把 School event 移到 `awaiting_cash_confirmation`；不把 payment request 改为 `paid`。页面已增加最小 Supabase Auth 登录区块，支持 email/password 登录、登出、显示当前登录邮箱；personal Cash 确认提交前必须有 School Supabase session，否则不调用 Edge Function。新增 result callback 基础：`school_personal_cash_payment_request_result_rpcs.sql` 定义 confirmed/rejected 回写 RPC，`supabase/functions/sync-cash-request-result/` 读取 Cash approved/rejected request 后回写 School。Function 当前只支持 personal `teacher_wage` JPY payment request，不接收入页；Cash approve/reject E2E 仍未执行。该手动同步器是验证/运维工具，不是最终日常业务入口。
- 最近关键更新: 2026-06-13 修正 v2 UI 方向并接入 Edge Function wrapper：不新增独立 School 同步入口。personal + `teacher_wage` + JPY 的支付动作在老师工资支付页面内选择 Cash 支付账户，按钮文案使用 `提交到 Cash 确认`，提交前要求用户先登录 School Supabase Auth，提交后调用 `request-cash-confirmation` 并提示 `已提交到 Cash System 待确认`。Cash approve 后通过 `sync-cash-request-result` 回写 School，才把 payment request 标为 `paid` 并把 linkage 标为 `synced`；Cash reject 后回写 School，payment request 保持 `pending`，linkage 标为 `cash_rejected`。本轮只加最小登录/session bridge、前端 guard 和文档，不部署 Function、不运行测试。
- 当前限制 / hard stop: 不删除 payment request、wage lock、expense、account transaction。青空塾确认支付路径必须保持现有 school account/expense/account transaction 行为。最小登录仅用于 Edge Function JWT，不是完整 operator/admin 权限模型，也不是学生/老师账号体系。最终 Cash linkage v2 目标不是 School 直接创建 Cash transaction，也不是普通用户进入独立同步页，而是老师工资支付页内选择 Cash 支付账户并提交 Cash 确认；approve 后才生成 Cash transaction 并改变 Cash 余额，reject 后不生成 Cash transaction 且 School 显示 `Cash已拒绝` 或保持未支付。继续排除 Cash CNY、报销、法人账户、学费收入、兼职工资、撤销同步、自动后台任务、跨 DB 强事务或历史 backfill。已 synced 的个人业务支付若要撤销，应继续阻断，直到 reversal sync 独立实现。
- 下一步: 未来改 payment status actions 时，显式重测 cancel/restore/reissue 和 confirm/reverse 链路；如需真正开放支付记录编辑，必须先设计独立 edit RPC/API guard。personal Cash linkage v2 后续阶段是 SQL/Function apply/deploy/config、ROLLBACK/COMMIT 白名单 approve/reject E2E、5 月两笔 JPY 老师工资 approve/reject 试运行；仍不做跨 DB 强事务。
