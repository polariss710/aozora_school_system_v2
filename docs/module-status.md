...
| 老师工资支付 | V1 可用 + personal Cash Phase 1 E2E sync ready | Retest status actions before future changes; reversal sync / payment retry UI / automatic sync scheduling require separate guarded phases |
...
| 收入记录 | V1 可用 + Phase 2 personal tuition Cash linkage E2E complete + sync status/retry UI | Tuition reverse sync and automatic sync scheduling remain separate phases; sync operations are manual-script based |
...
- 最近关键更新: 2026-06-13 完成手动同步器 `scripts/sync-personal-cash-linkage.zsh` 并补充运维说明：读取 eligible pending outbox，调用 Cash `home_create_external_jpy_transaction`，成功后通过 `school_update_personal_cash_linkage_event_status` 回写 `synced` 和 Cash transaction id，失败时回写 `failed` 和错误信息。脚本支持 `--limit`、`--event-id`、`--dry-run`，同一个入口也处理 Phase 2 tuition income linkage。白名单 E2E 成功测试：Cash account `94000000-0000-4000-8000-000000150501`、school payment request `94000000-0000-4000-8000-000000150101`、school event `2f20e264-1e20-493d-b92a-58c244abfa09`、Cash transaction `fbd3e5df-14be-4b3b-9a0b-319f4416968b`。重复执行未重复生成 Cash 流水；失败路径 event `11f8f9ee-cbf4-4a29-8d6a-dc56a7d2e7e4` 已进入 `failed`；旧 fake pending event `9b95e09a-09c4-4203-bc02-07daaf1beb5b` 已标记 `blocked`。
...
- 下一步: 未来改 payment status actions 时，显式重测 cancel/restore/reissue 和 confirm/reverse 链路；如需真正开放支付记录编辑，必须先设计独立 edit RPC/API guard。personal Cash linkage 后续阶段是 reversal sync、payment failed retry UI/操作面、自动调度、测试数据清理或正式配置迁移；仍不做跨 DB 强事务。
...
- 最近关键更新: 2026-06-13 完成 Phase 2 personal-business tuition income -> Cash System JPY income E2E linkage，并新增 Cash sync 状态/重试 UI 和手动同步运维说明。已执行 Cash guard SQL、school DB foundation SQL、create RPC、sync executor、income entry UI/API、income detail guard 和普通 edit/reverse RPC guard。支持路径为 personal business `tuition` JPY income -> `school_personal_cash_income_linkage_events` pending -> `scripts/sync-personal-cash-linkage.zsh` -> Cash `home_jpy_transactions.transaction_type = income` -> school event `synced`。收入详情页在存在 income linkage event 时显示 Cash sync 卡片：pending/synced/failed、Cash transaction id、Cash account snapshot、`synced_at`、`last_error`、`retry_count`、idempotency key；failed event 显示 `重新同步` 操作，将符合保护条件的 failed `tuition_income_received` event 重置为 pending，等待下一次 sync executor，不直接创建 Cash transaction；收入列表显示 `Cash待同步` / `Cash已同步` / `Cash失败` badge。当前同步仍是手动脚本，不在浏览器执行 zsh，不自动调度；运维命令以 `scripts/sync-personal-cash-linkage.zsh --dry-run`、`--limit`、`--event-id` 为入口。ROLLBACK 白名单测试验证 create/reject/edit-reverse guard；COMMIT E2E 验证 Cash transaction 数量保持 1、sync 幂等、school event synced 回写、无 `school_account_transactions`、不更新 `school_accounts.current_balance`；随后清理 Phase 2 E2E `codex-test-personal-cash-tuition-e2e-20260613` 残留，Cash target account/transaction、School target business/student/mapping/income/event、`home_cny_transactions` marker 均为 0。2026-06-12 收入新增/编辑字段统一；账户币种不再作为 dialog 输入，改由入账账户决定并由 RPC 校验。
