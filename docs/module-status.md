...
| 私塾打工 | Workflow V1 已安装 / 课程组进度已接入 | Browser-smoke copy/generate/settlement later with whitelist data only |
...
- 计算/权限: 课时工资和结算总额由 RPC 统一计算。开始时间、结束时间、回数为必填，课程组可选；RPC 根据时间差计算单次 planned / actual 课时。`course_group_name` 用于把同一组固定回数课程合并展示进度；回数、计划总课时、实际累计课时、剩余参考课时只用于课程进度查看，不进入工资结算；月度结算课时合计只取 actual 课时合计。交通费保存在 planned / actual 课时上且允许为 0；月度结算行保存时记录时给、调整额和总额，交通费从本月 actual 课时汇总；锁定时写入 settlement detail snapshots。所有 page-facing list/create/update/delete/generate/save/lock/request RPC 只 grant execute to `authenticated`，不 grant anon。
