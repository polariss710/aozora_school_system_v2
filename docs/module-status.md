...
- 编辑范围: 账户新增可填写 `account_code`, `name`, `account_type`, `currency`, `business_entity_id`, `is_company_account`, `is_active`, `note`; `opening_balance` 和 `current_balance` 固定为 `0`，不生成 `school_account_transactions`。账户资料编辑仅允许 `name`, `account_type`, `is_company_account`, `is_active`, `note`，且编辑 dialog 顶部 readonly summary 已隐藏。不可编辑/受保护字段包括 `account_code`, `currency`, `business_entity_id`, `opening_balance`, `current_balance`, `app_type`, `created_at`，以及 `school_account_transactions` 和历史收入、支出、报销、支付、转账、调整、结算、工资链路；余额修正只能走已验证的 account adjustment flow。
...
- 账户新增收缩限制: 2026-06-12 已复核但未将账户新增收缩为 `name/type/status/note` 四字段；当前 `school_accounts` schema/RPC 创建账户必须提供 `account_code`, `currency`, and `business_entity_id`，隐藏这些字段需要猜测编号、币种和业务归属默认值，可能影响后续收入、支出、支付、报销、账户流水和汇总链路，因此本轮只隐藏账户编辑 summary，不改账户新增字段。
...
- 老师 dialog 收缩: 2026-06-12 已完成老师新增/编辑字段统一收缩；两个 dialog 均使用中尺寸、桌面两列、移动端单列，隐藏老师编号/不可编辑字段/摘要区域，不再暴露显示名称、系统姓名、读音、默认时给、工资币种、支付币种或支付方式输入。目标字段均由现有 `school_teachers` schema 支持，本轮没有未接通字段，也没有新增 schema；新增/编辑通过 `school_create_teacher_profile(jsonb)` / `school_update_teacher_profile(uuid,jsonb)` RPC overload 写入。
- 业务归属默认币种验证: 2026-06-12 已只读确认 `school_business_entities.default_currency` 不参与学生结算、老师工资生成/快照、支付请求、报销、收入/支出、账户流水或利润汇总的金额计算/币种校验；这些链路使用源记录、账户、支付请求、结算或工资快照自身的 currency/amount/exchange-rate 字段。该字段保留在 DB 中作为兼容字段，但不再进入 v2 业务归属新增/编辑 dialog。
- 业务归属 dialog 收缩: 2026-06-12 已完成业务归属新增/编辑字段统一收缩为 `name`, `entity_type` (`company` = 公司业务, `personal` = 个人业务), `is_active`, `note`；两个 dialog 使用小尺寸，隐藏编号/default_currency/company-report/readonly summary。`school_create_business_entity_profile(jsonb)` 会生成系统 code 并设置隐藏 `default_currency = JPY`；`school_update_business_entity_profile(uuid,jsonb)` 保留既有 code/default_currency/is_company_report，只更新四个开放字段。
...
- 已知限制: parent/tuition-rule editing remains closed; student contact/guardian/birthday/profile expansion needs separate API/RPC/UI design with privacy and validation handling; teacher non-scoped contact/payment legacy fields and wage/payment defaults remain closed unless a separate privacy/payment-account design reopens them; business entity default currency remains hidden/retained unless future defaulting behavior requires reopening it through a dedicated design.
