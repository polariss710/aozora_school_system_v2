...
- 当前状态: V1 可用。支付列表可确认支付、反转已支付请求、取消 pending、恢复 cancelled、reissue reversed；详情页只读。个人业务 Cash System linkage Phase 1 已验证：Cash-side external JPY RPC、school-side mapping/outbox schema/RPC/API、payment confirm 到 school pending outbox、手动同步器写入 Cash JPY transaction 并回写 school synced。Cash linkage v2 的 School 侧 request lifecycle SQL/RPC 已 apply 并通过 ROLLBACK 验证；Edge Function 桥接代码已加入 `supabase/functions/request-cash-confirmation/`，但尚未部署、尚未接入老师工资支付页面，Cash approve/reject 后回写 School 仍未实现。该手动同步器是验证/运维工具，不是最终日常业务入口。
- 最近关键更新: 2026-06-13 修正 v2 UI 方向：不新增独立 School 同步入口。personal + `teacher_wage` + JPY 的支付动作应在老师工资支付页面内选择 Cash 支付账户，按钮文案使用 `提交到 Cash 确认` 或 `请求支付确认`，School 不应立即标记 `paid`；Cash approve 后才 `paid/Cash已确认`，Cash reject 后仍 `pending/Cash已拒绝` 并显示拒绝理由。Edge Function `request-cash-confirmation` 仍作为该业务按钮背后的安全桥接，只创建 Cash pending request，不创建 Cash transaction，不把 payment request 改为 `paid`。
