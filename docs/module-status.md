...
| 收入记录 | V1 可用 + Phase 2 personal tuition Cash linkage E2E complete + sync status/retry UI | Tuition reverse sync and automatic sync scheduling remain separate phases |
...
- 下一步: Tuition income reverse sync、自动调度 sync executor、部分收款、付款计划、旧流水修正、跨账户收入迁移、结算外费用是否进入学生账单，均需另开 guarded design。
