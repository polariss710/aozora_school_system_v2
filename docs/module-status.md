...
- 跨月待补课高优先级: 数据模型和统计口径可以表达 `lesson_type = actual` + `status = makeup_completed` + `is_billable = false`。`docs/lesson-cross-month-makeup-completion-design.md` 已完成 design-only 边界：跨月补课应选择原月份 `pending_makeup` planned，并在补课月份生成 `makeup_completed` actual，`planned_lesson_id` 指向原 planned，不复制 planned/actual，不修改来源 planned；原月份只读显示“已于补课月份完成”，补课月份只读显示“来源：原月份待补课”。后续实现建议新增专用 RPC，因为现有当月补课 RPC 会把 actual `year_month` 继承为来源 planned 月份。该功能仍未实现，需单独 guarded write workflow。
