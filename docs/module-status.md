...
- 当前状态: V1 可用。支付列表可确认支付、反转已支付请求、取消 pending、恢复 cancelled、reissue reversed；详情页只读。个人业务 Cash System linkage Phase 1 已验证：Cash-side external JPY RPC、school-side mapping/outbox schema/RPC/API、payment confirm 到 school pending outbox、手动同步器写入 Cash JPY transaction 并回写 school synced。该手动同步器是验证/运维工具，不是最终日常业务入口。
...
- 下一步: 未来改 payment status actions 时，显式重测 cancel/restore/reissue 和 confirm/reverse 链路；如需真正开放支付记录编辑，必须先设计独立 edit RPC/API guard。personal Cash linkage v2 后续阶段是 Cash DB pending request 表/RPC、Cash approve/reject UI、Edge Function 桥接、School payment lifecycle/UI 调整、5 月两笔 JPY 老师工资 approve/reject 试运行；仍不做跨 DB 强事务。
