...
- 当前状态: V1 可用。支付列表可确认支付、反转已支付请求、取消 pending、恢复 cancelled、reissue reversed；详情页只读。个人业务 Cash System linkage Phase 1 已验证：Cash-side external JPY RPC、school-side mapping/outbox schema/RPC/API、payment confirm 到 school pending outbox、手动同步器写入 Cash JPY transaction 并回写 school synced。Cash linkage v2 的 School 侧 request lifecycle SQL/RPC 已 apply 并通过 ROLLBACK 验证；老师工资支付页 personal + `teacher_wage` + JPY 的 pending 请求选择 Cash 支付账户后，通过 `js/api/payment-api.js` 的 `requestCashConfirmationViaFunction(...)` 调用 `request-cash-confirmation` Edge Function。该 Function 创建/复用 School linkage event，创建 Cash pending request，并把 School event 移到 `awaiting_cash_confirmation`；不把 payment request 改为 `paid`。新增 result callback 基础：`school_personal_cash_payment_request_result_rpcs.sql` 定义 confirmed/rejected 回写 RPC，`supabase/functions/sync-cash-request-result/` 读取 Cash approved/rejected request 后回写 School。Function 当前只支持 personal `teacher_wage` JPY payment request，不接收入页；尚未部署、尚未运行，Cash approve/reject E2E 仍未执行。该手动同步器是验证/运维工具，不是最终日常业务入口。
- 最近关键更新: 2026-06-13 修正 v2 UI 方向并接入 Edge Function wrapper：不新增独立 School 同步入口。personal + `teacher_wage` + JPY 的支付动作在老师工资支付页面内选择 Cash 支付账户，按钮文案使用 `提交到 Cash 确认`，提交后调用 `request-cash-confirmation` 并提示 `已提交到 Cash System 待确认`。Cash approve 后通过 `sync-cash-request-result` 回写 School，才把 payment request 标为 `paid` 并把 linkage 标为 `synced`；Cash reject 后回写 School，payment request 保持 `pending`，linkage 标为 `cash_rejected`。本轮只加代码/SQL/文档，不部署 Function、不运行测试。
