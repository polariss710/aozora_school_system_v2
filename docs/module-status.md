...
- 可写入功能: create planned lesson, create completed actual from planned, create cancelled actual from planned, makeup_completed actual from planned, planned-only batch import, guarded lesson edit, planned-only void. Cross-month makeup_completed actual RPC `school_create_cross_month_makeup_completed_actual_from_planned` is verified at DB/RPC level but not yet exposed through API/UI. All exposed page writes go through `js/api/lesson-api.js` and verified RPCs.
...
- 跨月待补课高优先级: 数据模型和统计口径可以表达 `lesson_type = actual` + `status = makeup_completed` + `is_billable = false`。`docs/lesson-cross-month-makeup-completion-design.md` 已完成设计边界；`school_create_cross_month_makeup_completed_actual_from_planned_rpc.sql` 已完成 DB/RPC phase。专用 RPC 选择原月份 `pending_makeup` planned，并在补课月份生成 `makeup_completed` actual，`planned_lesson_id` 指向原 planned，不复制 planned/actual，不修改来源 planned，默认 `is_billable = false`，按目标 actual 月检查学生结算锁和老师工资锁。API wrapper、UI 入口、原月份/补课月份引用显示仍未实现，需后续单独 guarded phase。
- 未完成: free actual creation outside planned flow, full actual batch import, lesson delete, void restore, `voided_by`, wage lock generation from lessons, settlement adjustment/generation beyond completed settlement V1, auto-matching by student/teacher/subject/date/time, same-file planned/actual linking, cross-month pending-makeup API/UI entry and cross-month reference display.
...
- 后续优先级: keep planned-only import as stable V1; next implement cross-month pending-makeup API wrapper, target-month UI entry, and source/target reference display as separate guarded phases; keep full actual/history migration import out of current implementation; design void restore and wage-lock generation only as separate guarded phases.
...
- 未完成: historical data migration/repair, full actual import, cross-month pending-makeup API/UI entry and reference display, multi-version settlement history, settlement adjustment, carryover automatic revoke/rebuild, wage lock generation, lesson delete/restore, wage rule physical delete, payment cancel/restore/reissue retest depth, whitelist test data cleanup.
...
- 后续优先级: first preserve completed V1 surfaces; next complete cross-month pending-makeup API/UI and reference display on top of the verified RPC; then handle full actual import/history migration, wage lock generation, and other backlog as separate phases.
