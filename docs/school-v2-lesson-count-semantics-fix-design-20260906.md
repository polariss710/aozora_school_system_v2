...
> ## ✅ 已于 2026-09-06 23:03:18 JST 部署到生产
>
> 六个函数在同一事务内 `COMMIT` 切换到 v2。部署脚本
> `sql/current/school_tuition_lesson_count_semantics_v2_deploy_20260906.sql`
> （commit `1c21ee3`），rehearsal 与 commit 各执行一次，各 4.2 秒，退出码 0。
>
> **部署后核验**：六函数 canonical md5 与回滚脚本内嵌常量逐一相等；
> ACL / owner / prosecdef / provolatile / proconfig 全部未变；
> 独立新连接 22/22 张账单校验通过；六张业务表行数与内容指纹前后完全相同
> （只动函数定义，零业务数据变更）；87 个函数签名中仅目标 6 个变化。
>
> **回滚**：`sql/current/school_tuition_lesson_count_semantics_v2_rollback_20260906.sql`，
> 六个 v2 基线已内嵌为经审查常量，闸门可用。尚无 v2 账单时用 `scope=full`，
> 已有 v2 账单后必须用 `scope=generation_only`。该脚本**从未实跑过**，
> 真要用先跑 `-v mode=rehearsal`。
>
> **验证证据**（均在 `~/aozora-security-20260827/`，非 git）：
> 部署 rehearsal 16:44、只读对比 17:14（216 组合 12 断言）、
> 写入验证 22:26（3 张真实 v2 账单 + 2 负例 + 零残留，已回滚）、
> 部署 23:03。
>
> ⚠️ **部署本身不生成任何账单、不开 gate、不做结算**。后续业务顺序见文末。
...
## 13. 部署之后的业务顺序（2026-09-06 记）

⚠️ **一旦 9 月账单生成成功，8 月结算即被视为 consumed，此后
`unlock` / `adjust` / `relock` 被 `TUITION_CONSUMED_SETTLEMENT_IMMUTABLE` 永久拒绝。**

1. ~~部署 lesson_count 修复~~ ✅ 已完成（23:03）
2. 补上欠着的 rollback-only **零结转 E2E**（月度结算前必须做）
3. 开 `student_tuition_generate` gate（当前 `blocked`；gate 在 `atomic_core` 之上）
4. **生成 `be7effdf-…` 的 2026-08 账单 → 应为 4 条 / JPY 72,000**
   （22:26 写入验证实测值；修复前会是 3 条 / 54,000）
   业务负责人确认那 18,000 是**应收未收**
5. 8 月月度结算定稿
6. 生成 9 月账单 —— **此刻 8 月的账焊死**

**主动承担的剩余风险**：部署后 2026-04 / 2026-06 三个组合
（36,000 / 32,500 / 14,875）从 `CANDIDATES_EMPTY` 变为**可生成**。
这几笔学费已由其他方式收讫，此前那个错误的 guard 恰好挡着它们（巧合，非设计）。
现在**只剩「不补建历史月份账单」这条操作纪律在挡**，技术上无阻拦。
业务负责人已明确不处理 4–6 月历史账单。

**防复发的两件收尾**（不在本次范围，但同一根因）：
新增课程时**回数设为必选项**（根因）；preview **显示被排除的候选与原因**
（静默少算的机制未变，且本次修复扩大了暴露面）。
