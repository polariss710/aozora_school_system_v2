...
- 部署状态：**已部署**（2026-08-24 更新）。Codex 于 2026-08-24 在 School 生产
  ...
  - 本文件撰写时（08-23）状态为「未部署」，部署后才补正。**本行记录的是历史动作，
    不构成当前状态的权威**——它自身就过期过一次。要确认现在是否已部署，请查生产
    （`contract_version` 是否为 `student_settlement_lesson_week_close_v2`），
    不要读本文件。理由见文末第 8 节第 5 项，规则已写入 `AGENTS.md`。
...
6. **共享资产的缓存键会静默陈旧。**
   本项目的缓存键是逐资源、逐阶段冻结的：每个页面携带的是它自己最后一次改动
   时的键。对只被一个页面引用的资源没问题，但对共享资产会失效——改了共享文件
   却只升了同时改动的那几个页面的键，其余页面继续吃缓存里的旧版本。

   2026-08-24 实测：112 个目标资源中 94 个只有一条入边，18 个有多条入边，
   其中 5 个多入边资源的键不一致。最严重的两个正是最共享的：
   `css/app.css`（32 条入边、约 10 种键）与 `js/config.js`（33 条入边、约 12 种键）。

   已造成一次真实陈旧：`0767630`（2026-08-21）在 `css/app.css` 的 `@media print`
   规则中加入 `#globalSessionBar` 以隐藏打印文档中的登录邮箱，但只升了三个对外
   打印页（合同书、报价单、学费收据）的键。`#globalSessionBar` 由
   `js/auth-guard.js` 注入、被 31 个 app 引用，而 **29 个页面仍挂 2026-08-04 的
   `p0-g1-a-20260804-1`**，对在 8/21 之前缓存过 `app.css` 的浏览器不含该规则。
   高风险面（真正对外打印的三页）已堵住，漏的是内部管理页，严重性有限。

   **不要试图用静态断言表达「同一目标的全部入边使用同一键」**——该性质当前在
   5 处不成立，要让它成立需改动 30 余个文件，且下次改共享资产又会重演。
   真正的解法是内容哈希键（`app.css?v=<sha>`）：文件一变哈希就变，所有引用自动
   一致，不依赖任何人记得升键。建议单独立项。

   同日据此决定：删除 4 个测试中钉死历史缓存键的断言，不建替代测试。那些断言
   守的是「某个已结束阶段当时的键」，本无防护价值；真正该守的不变式只有内容
   哈希能可靠实现。**此处留下的覆盖空洞是已知且有意的**，在内容哈希落地前，
   缓存失效正确性无自动化保障。
